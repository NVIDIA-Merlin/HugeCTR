...
def test_multi_table_sparse_input_hps():
    ...
    for i in range(len(inputs)):
        embeddings0_gt.append(tf.nn.embedding_lookup_sparse(params = embedding_weights_per_table[0], 
                                                            sp_ids=inputs[i][0], 
                                                            sp_weights = None, combiner="mean"))
        embeddings1_gt.append(tf.nn.embedding_lookup(params = embedding_weights_per_table[1], 
                                                    ids=inputs[i][1]))
        logit_gt_batch, _, _ = trained_model(inputs[i])
        logit_gt.append(logit_gt_batch)

    embeddings0 = np.array( tf.reshape( tf.concat(embeddings0, axis=0), [-1,])  )
    embeddings1 = np.array( tf.reshape( tf.concat(embeddings1, axis=0), [-1,])  )
    logit = np.array( tf.reshape( tf.concat(logit, axis=0), [-1,]) )
    embeddings0_gt = np.array( tf.reshape( tf.concat(embeddings0_gt, axis=0), [-1,])  )
    embeddings1_gt = np.array( tf.reshape( tf.concat(embeddings1_gt, axis=0), [-1,])  )
    ...
    assert mse2 <= 1e-6
    assert mse3 <= 1e-6
    print("HPS TF Plugin does well for embedding vector lookup, mse1: {}, mse2: {}, mse3: {}".format(mse1, mse2, mse3))
