...
def get_global_replica_id(comm_tool=None, var=None):
    def _strategy():
        def _get_current_replica_id_in_group_sync():
            ...
            if replica_ctx:
                ...
                replica_id = (distribute_lib.get_update_replica_id() if hps_lib.in_tensorflow2() 
    ...
    if comm_tool is None:
        raise RuntimeError("HPS can only works with "
                        "tf.distribute.Strategy, MPI, Horovod or single GPU.")
    
    if comm_tool not in CommToolSet:
        raise RuntimeError("HPS only works with tf.distribute.Strategy, "
...
def lookup(values, model_name, table_id, emb_vec_size, emb_vec_dtype = tf.float32):
    ...
    vector  = hps_lib.lookup(values=values,
