 ...
 private:
  ...
  void register_buffers() {
    for (auto& size : ar_sizes_) {
      ...
      for (size_t g = 0; g < num_gpus_; g++) {
        ib_comm_->set_ar_coll_buf<TypeEmbeddingComp>(handle, d_ar_buff_[g].get_ptr(), size, g);
      }
...
TEST(ib_comms_ar_test, float_8gpu_per_node_perf) {
  test_ib_comm_perf<float>({0, 1, 2, 3, 4, 5, 6, 7});
}
