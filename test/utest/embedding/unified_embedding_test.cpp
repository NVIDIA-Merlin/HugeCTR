...
template <template <typename, typename> typename EmbeddingType, typename Key, typename EmbeddingComp>
void unified_embedding_forward(const TestParams &test_param,
                               const std::vector<int> &device_list, const std::vector<size_t> &slot_size_array) {
  test::mpi_init();
  int numprocs = 1;
#ifdef ENABLE_MPI
  ...
  for (size_t id = 0; id < local_gpu_count; ++id) {
    ...
    push_back_sparse_tensor({test_param.batch_size_per_gpu * local_gpu_count, test_param.max_nnz_per_sample},
  ...
  std::unique_ptr<EmbeddingType<Key, EmbeddingComp>> embedding = std::make_unique<EmbeddingType<Key, EmbeddingComp>>(
        train_keys, evaluate_keys, embedding_param, resource_manager);
  ...
  host_input_keys.resize(test_param.train_steps);
  {
    ...
    for(size_t train_step_id = 0; train_step_id < test_param.train_steps; ++train_step_id) {
      SparseTensor<Key> tensor;
      buf->reserve({test_param.batch_size_per_gpu * local_gpu_count, test_param.max_nnz_per_sample}, test_param.slot_num, &host_input_keys[train_step_id]);
      host_input_keys.push_back(tensor);
    }
    buf->allocate();
    for(size_t train_step_id = 0; train_step_id < test_param.train_steps; ++train_step_id) {
      init_sparse_tensor(host_input_keys[train_step_id], test_param.feature_num_per_sample, false, test_param.fixed_length);
    }
  }
  ...
  for (size_t i = 0; i < test_param.train_steps; ++i) {
    ...
    for(size_t id = 0; id < local_gpu_count; ++id) {
      ...
      sparse_tensor_helper::cuda::copy_async(train_keys[id], host_input_keys[i], cudaMemcpyHostToDevice, local_gpu->get_stream());
    }
  ...
  if(test_param.reference_check) {
    ...
    std::vector<Tensor2<EmbeddingComp>> forward_result_from_gpu(test_param.train_steps); // buffer to save gpu embedding result
    {
      std::shared_ptr<GeneralBuffer2<HostAllocator>> host_buff = GeneralBuffer2<HostAllocator>::create();
      for(size_t i = 0; i < test_param.train_steps; ++i){
        host_buff->reserve({embedding_param.train_batch_size, embedding_param.slot_num, embedding_param.embedding_vec_size}, &forward_result_from_gpu[i]);
      }
      host_buff->allocate();
    }
    ...
    buf_bag.embedding = embedding_cpu.hash_table_value_tensors_;
    {
      ...
      for (size_t id = 0; id < local_gpu_count; id++) {
        ...
        host_blobs_buff->reserve({max_voc_size_per_gpu, embedding_param.embedding_vec_size}, &tensor);
    ...
    for (size_t i = 0; i < test_param.train_steps; ++i) {
      ...
      ASSERT_TRUE(compare_array(embedding_cpu.embedding_feature_tensors_.get_num_elements(), embedding_cpu.embedding_feature_tensors_.get_ptr(), forward_result_from_gpu[i].get_ptr(), test_param.eposilon));
...
TEST(unified_embedding_test, test_distributed_embedding_train_multi_gpu_reference_check) {
  ...
  unified_embedding_forward<DistributedSlotSparseEmbeddingHash, unsigned int, float>(param,
                                                 {0, 1}, {});
}
...
TEST(unified_embedding_test, test_localized_embedding_train_multi_gpu_reference_check) {
  ...
  unified_embedding_forward<LocalizedSlotSparseEmbeddingHash, unsigned int, float>(param,
                                                 {0, 1}, {});
}
