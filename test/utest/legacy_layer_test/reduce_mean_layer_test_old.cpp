...
template <typename T>
void reduce_mean_cpu(const T* input, T* output, std::vector<size_t> dims, size_t axis) {
  if (axis == 0) {
    if (dims.size() == 1) {
    ...
    for (size_t i = 0; i < dims[0]; i++) {
      for (size_t j = 0; j < dims[1]; j++) {
        ...
        output[i * dims[1] + j] /= dims[2];
      }
    }
  }
}
template <typename T>
void reduce_mean_dgrad_cpu(const T* top_grad, T* dgrad, std::vector<size_t> dims, size_t axis) {
  if (axis == 0) {
    if (dims.size() == 2) {
      for (size_t j = 0; j < dims[1]; j++) {
        for (size_t i = 0; i < dims[0]; i++) {
          dgrad[i * dims[1] + j] = top_grad[j] / dims[0];
        }
      }
    } else if (dims.size() == 3) {
      for (size_t j = 0; j < dims[1]; j++) {
        for (size_t k = 0; k < dims[2]; k++) {
          for (size_t i = 0; i < dims[0]; i++) {
            dgrad[i * dims[1] * dims[2] + j * dims[2] + k] = top_grad[j * dims[2] + k] / dims[0];
          }
        }
      }
    }
    ...
    if (dims.size() == 2) {
      for (size_t i = 0; i < dims[0]; i++) {
        for (size_t j = 0; j < dims[1]; j++) {
          dgrad[i * dims[1] + j] = top_grad[i] / dims[1];
        }
      }
    } else if (dims.size() == 3) {
      for (size_t i = 0; i < dims[0]; i++) {
        for (size_t k = 0; k < dims[2]; k++) {
          for (size_t j = 0; j < dims[1]; j++) {
            dgrad[i * dims[1] * dims[2] + j * dims[2] + k] = top_grad[i * dims[2] + k] / dims[1];
          }
        }
      }
    }
    ...
    for (size_t i = 0; i < dims[0]; i++) {
      for (size_t j = 0; j < dims[1]; j++) {
        for (size_t k = 0; k < dims[2]; k++) {
          dgrad[i * dims[1] * dims[2] + j * dims[2] + k] = top_grad[i * dims[1] + j] / dims[2];
        }
      }
    }
  }
...
void reduce_mean_test(size_t batch_size, size_t slot_num, size_t embedding_vec_size, size_t axis) {
  ...
  ReduceMeanLayer<T> reduce_mean_layer(in_tensor, out_tensor, buff, axis, test::get_default_gpu());
...
TEST(reduce_mean_layer_old, fp32_2x3x4_0) { reduce_mean_test<float>(2, 3, 4, 0); }
TEST(reduce_mean_layer_old, fp32_2x3x4_1) { reduce_mean_test<float>(2, 3, 4, 1); }
TEST(reduce_mean_layer_old, fp32_2x3x4_2) { reduce_mean_test<float>(2, 3, 4, 2); }
TEST(reduce_mean_layer_old, fp32_40960x39x1_1) { reduce_mean_test<float>(40960, 39, 1, 1); }
