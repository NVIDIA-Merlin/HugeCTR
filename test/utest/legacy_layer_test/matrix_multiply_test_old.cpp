...
template <typename T>
void matmul_test_mixed(size_t m, size_t n, size_t g, size_t h) {
  size_t k = g * h;
  ...
  // fprop
  for (size_t i = 0; i < num; i++) {
    size_t size = in_tensors[i].get_num_elements();
    h_ins[i] = new T[size];
    h_bprop_out[i] = new T[size];
    h_cpu_bprop_out[i] = new T[size];
  ...
  matmul_cpu(h_ins[0], h_ins[1], h_cpu_out.get(), 1, m, n, k);
  ASSERT_TRUE(
      test::compare_array_approx<T>(h_out.get(), h_cpu_out.get(), out_size, Eps<T>::value()));
  ...
  matmul_dgrad_cpu(h_out.get(), h_ins.get(), h_cpu_bprop_out.get(), 1, m, n, k);
  for (size_t i = 0; i < num; i++) {
    size_t size = in_tensors[i].get_num_elements();
...
// 2D inputs
TEST(matmul_layer_old, fp32_1x2x3x4) { matmul_test<float>(1, 2, 3, 4); }
TEST(matmul_layer_old, fp32_1x128x256x32) { matmul_test<float>(1, 128, 256, 32); }
TEST(matmul_layer_old, fp32_1x256x512x1024) { matmul_test<float>(1, 256, 512, 1024); }
TEST(matmul_layer_old, fp32_1x1024x512x256) { matmul_test<float>(1, 1024, 512, 256); }
TEST(matmul_layer_old, fp32_1x1024x2048x1024) { matmul_test<float>(1, 1024, 2048, 1024); }

// 3D inputs
TEST(matmul_layer_old, fp32_2x2x3x4) { matmul_test<float>(2, 2, 3, 4); }
TEST(matmul_layer_old, fp32_32x128x256x32) { matmul_test<float>(32, 128, 256, 32); }
TEST(matmul_layer_old, fp32_64x256x512x1024) { matmul_test<float>(64, 256, 512, 1024); }
TEST(matmul_layer_old, fp32_12x1024x512x256) { matmul_test<float>(12, 1024, 512, 256); }
TEST(matmul_layer_old, fp32_6x1024x2048x1024) { matmul_test<float>(6, 1024, 2048, 1024); }

// 2Dx3D inputs
TEST(matmul_layer_old, mixed_fp32_8x4x4x3) { matmul_test_mixed<float>(8, 4, 4, 3); }

// 4D inputs
TEST(matmul_layer_old, fp32_2x2x2x3x4) { matmul_test_4d<float>(2, 2, 2, 3, 4); }
TEST(matmul_layer_old, fp32_32x4x128x256x32) { matmul_test_4d<float>(32, 4, 128, 256, 32); }
TEST(matmul_layer_old, fp32_12x10x1024x512x256) { matmul_test_4d<float>(12, 4, 1024, 512, 256); }
TEST(matmul_layer_old, fp32_4x1x1024x2048x1024) { matmul_test_4d<float>(4, 1, 1024, 2048, 1024); }
