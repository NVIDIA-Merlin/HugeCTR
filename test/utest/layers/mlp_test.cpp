...
static void cpu_mm(__half* c, const __half* a, bool transpose_a, const __half* b,
                   bool transpose_b, float beta, int m, int k, int n) {
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      float sum = 0.0f;
...
static void cpu_add_bias_and_re(__half* top, __half* middle, const __half* bias,
                                bool is_relu, int m, int n) {
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
...
static float compare_bit_array(const __half* arr1, const __half* arr2,
                               size_t n, float threshold, bool is_print) {
  size_t m = 0;
...
static void init_data_cpu(uint32_t* input_dims, uint32_t* output_dims, uint32_t n_layers, uint32_t batch_size) {
...
static void copy_data_from_cpu(uint32_t* input_dims, uint32_t* output_dims,
                               uint32_t n_layers, uint32_t batch_size) {
...
static void mlp_test(uint32_t* input_dims, uint32_t* output_dims, int* head_body_tail,
  ...
  fc_layers[0] = new FusedReluBiasFullyConnectedLayer(
      ...
      relu[0], false, std::vector<Initializer_t>(), config_set.async_mlp_wgrad,
...
//int out_dims[8] = {512, 256, 128, 1024, 1024, 512, 256, 1};
      ...
      inter_layer = new InteractionLayer<__half>(in_mlp_tensor, in_emb_tensor, out_tensor,
    ...
    if (2 == head_body_tail[i]) { // tail
        fc_layers[i] = new FusedReluBiasFullyConnectedLayer(
            ...
            config_set.async_mlp_wgrad, DenseLayerSwitchs(fuse_wb[i]));
  ...
  if (!config_set.is_perf_test) {
    ...
    for (uint32_t i = 0; i < n_layers; i++) {
      if(i > 0 && input_dims[i] != output_dims[i-1]){
        CK_CUDA_THROW_(cudaMemcpy(train_in_tensor[i].get_ptr(), h_bottom[i],
    ...
    for (uint32_t i = 0; i < n_layers; i++) {
      ASSERT_LE(check_data_cpu_and_gpu(h_top[i], train_out_tensor[i].get_ptr(),
                ...
                0.0)
          << "Forward, Y of the " << i << "th layer cross_check result fail" << endl;
    }
    // initialize dX
    ...
    for (int i = n_layers - 1; i >= 0; i--) {
      ASSERT_LE(
          ...
          0.0)
          << "Backward, dBias of the " << i << "th layer cross_check result fail" << endl;
      ASSERT_LE(check_data_cpu_and_gpu(h_kernel_grad[i],
                                       fc_layers[i]->get_weights_grad_tensor()[0].get_ptr(),
                                       input_dims[i] * output_dims[i], 1e-3),
                0.0)
          << "Backward, dW of the " << i << "th layer cross_check result fail" << endl;
      if (i > 0 && input_dims[i] != output_dims[i-1]) {
        break;
      }
    }
    ...
    for (size_t test_loop_idx = config_set.test_loop_cnt; test_loop_idx > 0; --test_loop_idx) {
      ...
      for (int bprop_idx = layer_end - 1; bprop_idx >= layer_start; --bprop_idx) {
        if (config_set.use_nvtx) {
          nvtx_str = to_string(bprop_idx);
          nvtxRangePush(nvtx_str.c_str());
        }
        for (size_t layer_loop_idx = 0; layer_loop_idx < config_set.layer_loop_cnt; ++layer_loop_idx) {
...
TEST(mlp_test, all) { mlp_test(in_dims, out_dims, head_body_tail, is_relu, fuse_wb, 8, 640, config_set); };
