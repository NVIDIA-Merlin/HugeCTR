...
#include "HugeCTR/include/trainable_layer.hpp"
 ...
 */
template <typename T>
class DummyTrainableLayer : public TrainableLayer<T> {
...
template <typename T>
DummyTrainableLayer<T>::DummyTrainableLayer(
    ...
    const Tensor2<T>& out_tensor, const std::shared_ptr<GPUResource>& gpu_resource,
...
void trainable_layer_test(size_t batch_size, size_t in_dim, size_t out_dim) {
  ...
  std::shared_ptr<BufferBlock2<T>> weight_buff = buf->create_block<T>();
  std::shared_ptr<BufferBlock2<T>> wgrad_buff = buf->create_block<T>();

  Tensor2<T> input_tensor;
  buf->reserve({batch_size, in_dim}, &input_tensor);
  Tensor2<T> output_tensor;
  buf->reserve({batch_size, out_dim}, &output_tensor);

  DummyTrainableLayer<T> dummy_trainable_layer(master_weight_buff, weight_buff, wgrad_buff,
...
template <>
void trainable_layer_test<float>(size_t batch_size, size_t in_dim, size_t out_dim) {
  std::shared_ptr<GeneralBuffer2<CudaAllocator>> buf = GeneralBuffer2<CudaAllocator>::create();
  std::shared_ptr<BufferBlock2<float>> weight_buff = buf->create_block<float>();
  std::shared_ptr<BufferBlock2<float>> wgrad_buff = buf->create_block<float>();
  ...
  DummyTrainableLayer<float> dummy_trainable_layer(
      weight_buff, weight_buff, wgrad_buff, input_tensor, output_tensor, test::get_default_gpu());
...
TEST(trainable_layer, fp32_4096x1024x10) { trainable_layer_test<float>(4096, 1024, 10); }
TEST(trainable_layer, fp16_4096x1024x10) { trainable_layer_test<__half>(4096, 1024, 10); }