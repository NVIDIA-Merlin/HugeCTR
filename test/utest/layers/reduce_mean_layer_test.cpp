...
template <typename T>
void reduce_mean_cpu(const T* input, T* output, core23::Shape shape, size_t axis) {
  if (axis == 0) {
    if (shape.dims() == 1) {
      ...
      for (size_t k = 0; k < shape.size(1); k++) {
        output[k] = 0;
        for (size_t i = 0; i < shape.size(0); i++) {
          output[k] += input[i * shape.size(1) + k];
        }
        output[k] /= shape.size(0);
      }
    } else if (shape.dims() == 3) {
      for (size_t j = 0; j < shape.size(1); j++) {
        for (size_t k = 0; k < shape.size(2); k++) {
          output[j * shape.size(2) + k] = 0;
          for (size_t i = 0; i < shape.size(0); i++) {
            output[j * shape.size(2) + k] +=
                input[i * shape.size(1) * shape.size(2) + j * shape.size(2) + k];
          }
          output[j * shape.size(2) + k] /= shape.size(0);
        }
      }
    }
  } else if (axis == 1) {
    if (shape.dims() == 2) {
      for (size_t i = 0; i < shape.size(0); i++) {
        output[i] = 0;
        for (size_t j = 0; j < shape.size(1); j++) {
          output[i] += input[i * shape.size(1) + j];
        }
        output[i] /= shape.size(1);
      }
    } else if (shape.dims() == 3) {
      for (size_t i = 0; i < shape.size(0); i++) {
        for (size_t k = 0; k < shape.size(2); k++) {
          output[i * shape.size(2) + k] = 0;
          for (size_t j = 0; j < shape.size(1); j++) {
            output[i * shape.size(2) + k] +=
                input[i * shape.size(1) * shape.size(2) + j * shape.size(2) + k];
          }
          output[i * shape.size(2) + k] /= shape.size(1);
        }
      }
    }
  } else if (axis == 2) {
    for (size_t i = 0; i < shape.size(0); i++) {
      for (size_t j = 0; j < shape.size(1); j++) {
        output[i * shape.size(1) + j] = 0;
        for (size_t k = 0; k < shape.size(2); k++) {
          output[i * shape.size(1) + j] +=
              input[i * shape.size(1) * shape.size(2) + j * shape.size(2) + k];
        }
        output[i * shape.size(1) + j] /= shape.size(2);
      }
    }
  }
}
template <typename T>
void reduce_mean_dgrad_cpu(const T* top_grad, T* dgrad, core23::Shape shape, size_t axis) {
  if (axis == 0) {
    if (shape.dims() == 2) {
      for (size_t j = 0; j < shape.size(1); j++) {
        for (size_t i = 0; i < shape.size(0); i++) {
          dgrad[i * shape.size(1) + j] = top_grad[j] / shape.size(0);
        }
      }
    } else if (shape.dims() == 3) {
      for (size_t j = 0; j < shape.size(1); j++) {
        for (size_t k = 0; k < shape.size(2); k++) {
          for (size_t i = 0; i < shape.size(0); i++) {
            dgrad[i * shape.size(1) * shape.size(2) + j * shape.size(2) + k] =
                top_grad[j * shape.size(2) + k] / shape.size(0);
          }
        }
      }
    }
    ...
    if (shape.dims() == 2) {
      for (size_t i = 0; i < shape.size(0); i++) {
        for (size_t j = 0; j < shape.size(1); j++) {
          dgrad[i * shape.size(1) + j] = top_grad[i] / shape.size(1);
        }
      }
    } else if (shape.dims() == 3) {
      for (size_t i = 0; i < shape.size(0); i++) {
        for (size_t k = 0; k < shape.size(2); k++) {
          for (size_t j = 0; j < shape.size(1); j++) {
            dgrad[i * shape.size(1) * shape.size(2) + j * shape.size(2) + k] =
                top_grad[i * shape.size(2) + k] / shape.size(1);
          }
        }
      }
    }
    ...
    for (size_t i = 0; i < shape.size(0); i++) {
      for (size_t j = 0; j < shape.size(1); j++) {
        for (size_t k = 0; k < shape.size(2); k++) {
          dgrad[i * shape.size(1) * shape.size(2) + j * shape.size(2) + k] =
              top_grad[i * shape.size(1) + j] / shape.size(2);
        }
      }
    }
  }
...
template <typename T>
void reduce_mean_test(int64_t batch_size, int64_t slot_num, int64_t embedding_vec_size,
                      int64_t axis) {
  ...
  ReduceMeanLayer<T> reduce_mean_layer(bottom_tensor, top_tensor, axis, test::get_default_gpu());

  reduce_mean_layer.initialize();
  ...
  reduce_mean_cpu<T>(h_bottom.data(), h_cpu_top.data(), in_shape, axis);
  ...
  reduce_mean_dgrad_cpu<T>(h_top.data(), h_bottom.data(), in_shape, axis);
...
TEST(reduce_mean_layer, fp32_2x3x4_0) { reduce_mean_test<float>(2, 3, 4, 0); }
TEST(reduce_mean_layer, fp32_2x3x4_1) { reduce_mean_test<float>(2, 3, 4, 1); }
TEST(reduce_mean_layer, fp32_2x3x4_2) { reduce_mean_test<float>(2, 3, 4, 2); }
TEST(reduce_mean_layer, fp32_40960x39x1_1) { reduce_mean_test<float>(40960, 39, 1, 1); }
