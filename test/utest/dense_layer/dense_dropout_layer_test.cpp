...
void dense_dropout_layer_test() {
  ...
  Layer_t layer_type = Layer_t::Dropout;
  std::vector<std::string> bottom_names{bottom_tensor_entity.name};
  std::vector<std::string> top_names = {"dropout1"};

  DenseLayer dense_layer(layer_type, bottom_names, top_names);
  test::dense_layer_test_common<DropoutLayer<T>>(
      {bottom_tensor_entity}, dense_layer, use_mixed_precision, false, false, 1024.f, false, false);
}

}  // namespace

TEST(test_dense_layer, dropout_layer_fp32) { dense_dropout_layer_test<float>(); }
TEST(test_dense_layer, dropout_layer_fp16) { dense_dropout_layer_test<__half>(); }