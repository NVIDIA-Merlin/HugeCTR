...
void dense_slice_layer_test() {
  ...
  Layer_t layer_type = Layer_t::Slice;
  std::vector<std::string> bottom_names{bottom_tensor_entity.name};
  std::vector<std::string> top_names = {"slice11", "slice12"};

  DenseLayer dense_layer(layer_type, bottom_names, top_names);
  dense_layer.ranges = {{0, 64}, {64, 128}};
  test::dense_layer_test_common<SliceLayer<T>>(
      {bottom_tensor_entity}, dense_layer, use_mixed_precision, false, false, 1024.f, false, false);
}

}  // namespace

TEST(test_dense_layer, slice_layer_fp32) { dense_slice_layer_test<float>(); }
TEST(test_dense_layer, slice_layer_fp16) { dense_slice_layer_test<__half>(); }