...
def on_single_gpu(args):
    ...
    dataset = tf_dataset(
        samples, labels, batchsize=args.global_batch_size, to_sparse_tensor=False, repeat=1
    )
    dataset = dataset.apply(
        tf.data.experimental.prefetch_to_device(device="/GPU:0", buffer_size=32)
    )
    ...
    with strategy.scope():
        ...
        model = DemoModel(
            max_vocabulary_size_per_gpu=args.vocabulary_size,
...
def on_multi_gpu(args):
    ...
    os.environ["TF_CONFIG"] = json.dumps(
        {
            "cluster": {"worker": ["localhost" + ":" + str(port + i) for i in range(args.gpu_num)]},
    ...
    dataset = tf_dataset(
        samples,
        labels,
        batchsize=args.global_batch_size // args.gpu_num,
        to_sparse_tensor=False,
        repeat=1,
    )
    dataset = dataset.apply(
        tf.data.experimental.prefetch_to_device(device="/GPU:0", buffer_size=tf.data.AUTOTUNE)
    )
    ...
    def _train_step(inputs, labels):
        ...
        reduced_grads = replica_ctx.all_reduce("sum", other_grads, options=comm_options)
        dense_opt.apply_gradients(
            zip(reduced_grads, other_var), experimental_aggregate_gradients=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nnz_per_slot", type=int, required=True, help="the number of valid keys in each slot"
    )
    parser.add_argument(
        "--embedding_vec_size", type=int, required=True, help="the size of embedding vector"
    )
    parser.add_argument(
        "--num_dense_layers",
        type=int,
    ...
    parser.add_argument(
        "--early_stop_iter", type=int, required=True, help="early stop at which iter"
    )
