...
class CriteoTsvReader:
    ...
    def __call__(self, input_ctx: tf.distribute.InputContext = None) -> tf.data.Dataset:
        batch_size = (
            input_ctx.get_per_replica_batch_size(self._batch_size)
            if input_ctx
        ...
        if self._sharding and input_ctx and input_ctx.num_input_pipelines > 1:
            filenames = filenames.shard(input_ctx.num_input_pipelines, input_ctx.input_pipeline_id)
        ...
        dataset = indices.interleave(map_func=make_dataset, num_parallel_calls=tf.data.AUTOTUNE)
...
if __name__ == "__main__":
    dataset = CriteoTsvReader(
        file_pattern=r"./train/*",
