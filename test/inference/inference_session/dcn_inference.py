...
def dcn_inference(config_file, model_name, data_path, use_gpu_embedding_cache):
    ...
    if use_gpu_embedding_cache:
        print(
            "[HUGECTR][INFO] Use gpu embedding cache, prediction number samples: {}, accuracy: {}".format(
                len(labels), accuracy
            )
        )
    else:
        print(
            "[HUGECTR][INFO] Use cpu parameter server, prediction number samples: {}, accuracy: {}".format(
                len(labels), accuracy
            )
        )
...
def calculate_accuracy(labels, output):
    ...
    flags = [
        1 if ((labels[i] == 0 and output[i] <= 0.5) or (labels[i] == 1 and output[i] > 0.5)) else 0
