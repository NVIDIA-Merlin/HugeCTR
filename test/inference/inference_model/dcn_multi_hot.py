...
model.fit(
    max_iter=2300, display=200, eval_interval=2000, snapshot=2000, snapshot_prefix="/dump_infer/dcn"
)
...
inference_model = InferenceModel("/dump_infer/dcn.json", inference_params)
...
if mse > 1e-3:
    ...
    print(
        "DCN multi hot inference results are consistent with those during training, mse: {}".format(
            mse
        )
    )
