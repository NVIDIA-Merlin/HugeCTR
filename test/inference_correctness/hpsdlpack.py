...
key = [1, 278211, 693281, 942339, 961915, 976114, 983029, 1001594, 1001623, 1007956, 1009214, 1009260, 1235430, 1315962, 1388251, 1388257, 1390426, 1398023, 1398084, 1398088, 1399012, 1399026, 1648645, 1817619, 2061099, 2129335, 2138480, 2138556]

def hps_dlpack(model_name, embedding_file_list, data_file,enable_cache):

    batch_size =1 
    CATEGORICAL_COLUMNS=["C1_C2","C3_C4"] + ["C" + str(x) for x in range(1, 27)]#["C1_C2","C3_C4"]+["C" + str(x) for x in range(1, 27)]
...
#test_df=pd.read_csv("/wdl_train/infer_test.csv",sep=',')
    ...
    print("[HUGECTR][INFO] The shape of vectors that lookup from native hps interface for embedding table 1 of wdl: {}, the vectors: {}".format(embedding1[0].shape, embedding1))
    print("[HUGECTR][INFO] The shape of vectors that lookup from native hps interface for embedding table 2 of wdl: {}, the vectors: {}\n\n".format(embedding2[0].shape, embedding2))
...
#4. Look up from pytorch dlpack
    ...
    print("[HUGECTR][INFO] The device type of embedding keys that lookup dlpack from hps interface for embedding table 2 of wdl: {}, the keys: {}".format(key.device, key))
    ...
    print("[HUGECTR][INFO] The device type of embedding vectors that lookup dlpack from hps interface for embedding table 2 of wdl: {}, the vectors: {}\n".format(out_put.device, out_put))
...
#5. Look up from tf dlpack
    ...
    with tf.device('/CPU:0'):
        ...
        print("[HUGECTR][INFO] The device type of embedding keys that lookup dlpack from hps interface for embedding table 1 of wdl: {}, the keys: {}".format(key_tensor.device, key_tensor))
