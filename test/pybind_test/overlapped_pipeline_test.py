...
model.add(
    hugectr.GroupDenseLayer(
        group_layer_type=hugectr.GroupLayer_t.GroupFusedInnerProduct,
...
model.add(
    hugectr.DenseLayer(
        layer_type=hugectr.Layer_t.Add, bottom_names=["fc3", "fc6"], top_names=["add"]
    )
)
model.add(
    hugectr.GroupDenseLayer(
...
model.summary()
model.fit(
    max_iter=601, display=100, eval_interval=100, snapshot=2000000, snapshot_prefix="overlapped"
)
