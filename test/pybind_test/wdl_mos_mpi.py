...
def model_oversubscriber_test(json_file, output_dir):
  ...
  mos = hugectr.CreateMOS(ps_types = [hugectr.TrainPSType_t.Staged, hugectr.TrainPSType_t.Cached],
                        sparse_models = [output_dir + "/wdl_0_sparse_model", output_dir + "/wdl_1_sparse_model"],
                        local_paths = [output_dir+"_1", output_dir+"_2"], hmem_cache_configs = [hc_cnfg])
