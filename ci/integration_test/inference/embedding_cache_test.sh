...
ID=$(docker run --net=host -v /mnt/nvdl/usr/aleliu/inference_ci/model_repository:/models -v /mnt/nvdl/usr/aleliu/inference_ci/:/hugectr/test/utest/ -u root -d ${CONT} bash -cx "cd /workdir/build/bin && mkdir -p /hugectr/Test_Data/rockdb && ./embedding_cache_test || exit 1")
docker logs -f $ID