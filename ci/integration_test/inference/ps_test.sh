...
ID=$(docker run --gpus=all --net=host -v /home/hugectr-ci/data/inference_ci/model_repository:/models -u root -d ${CONT} bash -cx "cd /workdir/build/bin && mkdir -p /hugectr/Test_Data/rockdb &&  ./parameter_server_test || exit 1 ")
docker logs -f $ID
