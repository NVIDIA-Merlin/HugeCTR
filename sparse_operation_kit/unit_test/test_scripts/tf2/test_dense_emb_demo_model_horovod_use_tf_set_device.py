...
def generate_vocabulary_table(vocabulary_size_per_gpu, embedding_vec_size, num_of_gpu):
    tensors = [
        tf.random.normal([vocabulary_size_per_gpu, embedding_vec_size]) for _ in range(num_of_gpu)
    ]
    return tensors
...
def get_args():
    ...
    parser.add_argument("--max_vocabulary_size_per_gpu", type=int, required=False, default=8192)
    parser.add_argument(
        "--slot_num", type=int, help="the number of feature fields", required=False, default=100
    )
    parser.add_argument(
        "--nnz_per_slot",
        type=int,
...
if __name__ == "__main__":
    ...
    if args.mixed_precision:
        policy = tf.keras.mixed_precision.Policy("mixed_precision")
