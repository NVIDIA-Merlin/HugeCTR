...
from test_dense_emb_demo_model_single_worker import (
    SOKDenseDemo,
    test_tf_dense_model,
...
def test_sok_dense_demo(args, init_tensors, *random_samples):
    ...
    os.environ["TF_CONFIG"] = json.dumps(
        {
            "cluster": {
                "worker": [args.ips[i] + ":" + str(port + i) for i in range(args.worker_num)]
            },
    ...
    if 1 == args.restore_params:
        ...
        sok_saver.load_embedding_values(
            sok_dense_demo.embedding_layer.embedding_variable, init_tensors
        )
        ...
        if "plugin" not in args.optimizer:
            with sok.OptimizerScope(embedding_variables):
                emb_opt.apply_gradients(
                    zip(emb_grads, embedding_variables), experimental_aggregate_gradients=False
                )
        else:
            emb_opt.apply_gradients(
                zip(emb_grads, embedding_variables), experimental_aggregate_gradients=False
    ...
    def _dataset_fn(input_context):
        ...
        dataset = utils.tf_dataset(
            *random_samples, batchsize=replica_batch_size, to_sparse_tensor=False, repeat=1
        )
        return dataset
...
def compare_dense_emb_sok_with_tf(args):
    if args.global_batch_size % args.local_gpu_num != 0:
        raise ValueError(
            "global_batch_size: %d is not divisible by local_gpu_num: %d"
            % (args.global_batch_size, args.local_gpu_num)
        )
    if args.global_batch_size % args.worker_num != 0:
        raise ValueError(
            "global_batch_size: %d is not divisible by worker_num: %d"
    ...
    if args.generate_new_datas:
        if args.use_hashtable:
            vocabulary_size = (
                args.local_gpu_num * args.max_vocabulary_size_per_gpu * args.worker_num
            )
        else:
            vocabulary_size = args.max_vocabulary_size_per_gpu

        worker_batch_size = args.global_batch_size // args.worker_num
        random_samples_local = utils.generate_random_samples(
            num_of_samples=worker_batch_size * args.iter_num,
        ...
        utils.save_to_file(
            r"./random_samples_" + str(args.task_id) + r".file", *random_samples_local
        )
    else:
        random_samples_local = utils.restore_from_file(
            r"./random_samples_" + str(args.task_id) + r".file"
        )
    ...
    sok_results_local, embedding_variable_name = test_sok_dense_demo(
        args, init_tensors, *random_samples_local
    )
    # save the forward embedding vector from different worker to file
    utils.save_to_file(
        r"./sok_embedding_vectors_" + str(args.task_id) + r".file", *sok_results_local
    )
    ...
    dataset_filenames = [
        r"./random_samples_" + str(task_id) + r".file" for task_id in range(args.worker_num)
    ]
    ...
    sok_results_filenames = [
        r"./sok_embedding_vectors_" + str(task_id) + r".file" for task_id in range(args.worker_num)
    ]
    ...
    if len(sok_results_total[0]) != len(tf_results):
        raise ValueError(
            "The length of results obtained from sok: %d is not equal to that obtained from TF: %d"
    ...
    for i in range(args.iter_num):
        if args.local_gpu_num != 1:
            sok_vector = tf.concat(
                [
                    tf.concat(sok_results_total[task_id][i].values, axis=0)
            ...
            sok_vector = tf.concat(
                [sok_results_total[task_id][i] for task_id in range(args.worker_num)], axis=0
            )
    ...
    print(
        ...
        ", with mixed_precision = %s" % (args.iter_num, args.mixed_precision)
    )
...
def get_task_id(ips):
    ...
    for i in range(len(ips)):
        if ips[i] == local_ip:
            return i
    raise ValueError("Cannot find local_ip: %s in ips list: [%s]" % (local_ip, ", ".join(ips)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_vocabulary_size_per_gpu", type=int, required=False, default=128)
    parser.add_argument(
        "--slot_num", type=int, help="the number of feature fields", required=False, default=1
    )
    parser.add_argument(
        "--nnz_per_slot",
        type=int,
    ...
    if utils.all_ips_in_local(args.ips):
        ...
        for task_id in range(args.worker_num):
            available_gpus = ",".join(
                [str(args.local_gpu_num * task_id + i) for i in range(args.local_gpu_num)]
            )
            print("[INFO]: on task: %d, its available GPUs are: %s" % (task_id, available_gpus))
            os.environ["CUDA_VISIBLE_DEVICES"] = available_gpus
            process = utils.TestProcess(
                func=compare_dense_emb_sok_with_tf, task_id=task_id, arguments=args
            )
            process.start()
