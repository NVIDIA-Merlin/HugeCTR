...
void UnitTester::test_all_gather_dispatcher(const size_t rows_num_per_sample,
    ...
    replica_context->set_input("replica_values", 
            TFTensorWrapper::create(const_cast<tensorflow::Tensor*>(values_tensor)),
            /*overwrite=*/true);
    replica_context->set_input("replica_indices",
...
void UnitTester::test_csr_conversion_distributed(const size_t global_replica_id,
    ...
    replica_context->set_input("total_values", 
                               TFTensorWrapper::create(const_cast<tensorflow::Tensor*>(values_tensor)),
                               /*overwrite=*/true);
    replica_context->set_input("total_row_indices", 
                               TFTensorWrapper::create(const_cast<tensorflow::Tensor*>(row_indices_tensor)),
                               /*overwrite=*/true);
    replica_context->set_input("dev_total_num_elements", 
                               TFTensorWrapper::create(const_cast<tensorflow::Tensor*>(num_elements_tensor)),
                               /*overwrite=*/true);
    replica_context->set_input("host_total_num_elements",
...
void UnitTester::test_reduce_scatter_dispatcher(const size_t global_replica_id, 
    ...
    replica_context->set_input("embedding_features",
                               ...
                               /*overwrite=*/true);
    replica_context->set_output("replica_output", 
                                TFTensorWrapper::create(output),
                                /*overwrite=*/true);
...
void UnitTester::try_allocate_memory(const size_t local_replica_id) const {
    ...
    if (!allocated) {
        ...
        auto helper = []() {allocated = true; MESSAGE("Allocated unit tester buffer."); };
