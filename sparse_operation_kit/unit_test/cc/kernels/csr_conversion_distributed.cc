...
public:
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_output(0, {global_batch_size_ * slot_num_ * max_nnz_}, &replica_values_tensor));
        Tensor* replica_csr_row_offsets_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->allocate_output(1, {global_batch_size_ * slot_num_ + 1}, &replica_csr_row_offsets_tensor));
        ...
        try {
            ...
            unit_tester->test_csr_conversion_distributed(global_replica_id_tensor->scalar<int32_t>()(),
                                                         static_cast<size_t>(global_batch_size_),
                                                         static_cast<size_t>(slot_num_),
