...
public:
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        OP_REQUIRES(ctx, TensorShapeUtils::IsVector(values_tensor->shape()), errors::Aborted("values must be vector."));
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_output(0, {global_batch_size_ * rows_num_per_sample_ * max_nnz_}, &values_out_tensor));
        Tensor* indices_out_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->allocate_output(1, {global_batch_size_ * rows_num_per_sample_ * max_nnz_}, &indices_out_tensor));
        Tensor* num_elements_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->allocate_output(2, {num_replicas_in_sync_tensor->scalar<int32_t>()()}, &num_elements_tensor));
        ...
        try {
            ...
            unit_tester->test_all_gather_dispatcher(static_cast<size_t>(rows_num_per_sample_),
                                                    static_cast<size_t>(max_nnz_),
                                                    static_cast<size_t>(global_batch_size_),
