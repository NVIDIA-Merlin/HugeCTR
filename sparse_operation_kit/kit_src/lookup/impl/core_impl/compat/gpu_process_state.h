...
class GPUProcessState {
 public:
  ...
  bool HasGPUDevice() const NO_THREAD_SAFETY_ANALYSIS { return gpu_device_enabled_; }

  // Set the flag to indicate a GPU device has been created.
  // Disable thread safety analysis since a race is benign here.
  void EnableGPUDevice() NO_THREAD_SAFETY_ANALYSIS { gpu_device_enabled_ = true; }
  ...
  virtual Allocator* GetGPUAllocator(const GPUOptions& options, TfGpuId tf_gpu_id,
                                     size_t total_bytes);
  ...
  virtual void AddGPUAllocVisitor(int bus_id, const SubAllocator::Visitor& visitor);
  ...
  virtual void AddGpuHostAllocVisitor(int numa_node, const SubAllocator::Visitor& visitor);

  // Registers a Visitor to be invoked on each chunk handed back for freeing to
  // the SubAllocator of the GpuHostAllocator for the given numa_node.
  virtual void AddGpuHostFreeVisitor(int numa_node, const SubAllocator::Visitor& visitor);
 ...
 protected:
  ...
  std::vector<std::vector<SubAllocator::Visitor>> gpu_host_alloc_visitors_ GUARDED_BY(mu_);
  std::vector<std::vector<SubAllocator::Visitor>> gpu_host_free_visitors_ GUARDED_BY(mu_);
};
