...
: tensorflow::TensorBuffer(nullptr), tensor_(nullptr)
{}
...
size_t EmbeddingBuffer::size() const {
    return tensor_->get_size_in_bytes();
}

tensorflow::TensorBuffer* EmbeddingBuffer::root_buffer() {
    return this;
}
...
std::shared_ptr<EmbeddingBufferBuilder> EmbeddingBufferBuilder::create(std::shared_ptr<Tensor> tensor) {
