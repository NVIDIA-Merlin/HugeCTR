...
const std::unordered_map<std::string, SparseEmbeddingType> SparseEmbeddingTypeMap = {
    {"distributed", SparseEmbeddingType::Distributed},
    {"localized", SparseEmbeddingType::Localized}}; 
const std::unordered_map<std::string, DenseEmbeddingType> DenseEmbeddingTypeMap = {
    {"custom", DenseEmbeddingType::Custom}};
const std::unordered_map<std::string, CombinerType> CombinerMap = {
    {"sum", CombinerType::Sum},
    {"mean", CombinerType::Mean}};
const std::unordered_map<std::string, OptimizerType> OptimizerMap = {
    {"Adam", OptimizerType::Adam}};


std::shared_ptr<OptimizerHyperParamsHandler> OptimizerHyperParamsHandler::create(
                                    optimizer_hyper_params&& hyper_params) {
    return std::shared_ptr<OptimizerHyperParamsHandler>(new OptimizerHyperParamsHandler(std::move(hyper_params)));
}
...
float OptimizerHyperParamsHandler::get_hyper_param(const std::string param_name) const {
    ...
    if (hyper_params_.end() == iter) throw std::runtime_error(ErrorBase +
        "Cannot find hyper param whose name is: " + param_name);
    return iter->second;
}
...
CudaDeviceContext::CudaDeviceContext() {
    CK_CUDA(cudaGetDevice(&original_device_));
}

CudaDeviceContext::CudaDeviceContext(int32_t device) 
: CudaDeviceContext() 
{
...
CudaDeviceContext::~CudaDeviceContext() noexcept(false) {
    set_device(original_device_);
}

void CudaDeviceContext::set_device(int32_t device) const {
