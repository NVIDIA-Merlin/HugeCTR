...
Context::Context(const size_t global_replica_id) 
: global_replica_id_(global_replica_id)
{}
...
template <typename T>
void Context::set_output(const std::string name, const HugeCTR::Tensor2<T>& tensor, const bool overwrite) {
    try {
        if (!tensor.allocated()) throw std::runtime_error("Invalid tensor whose name is " + name);
        auto iter = tensors_.find(name);
        if (tensors_.end() != iter) {
            if (overwrite) iter->second = Tensor2Wrapper<T>::create(const_cast<HugeCTR::Tensor2<T>&>(tensor));
        } else {
            tensors_.emplace(std::make_pair(name, Tensor2Wrapper<T>::create(const_cast<HugeCTR::Tensor2<T>&>(tensor))));
        }
    } catch (const std::exception& error) {
        throw std::runtime_error(ErrorBase + error.what());
    }
...
template void Context::record_internal_tensor(const std::string name, HugeCTR::Tensor2<size_t>& tensor, 
                                              const bool overwrite);
template void Context::record_internal_tensor(const std::string name, HugeCTR::Tensor2<int64_t>& tensor, 
                                              const bool overwrite);
...
std::shared_ptr<Tensor>& Context::get_internal_tensor(const std::string name) {
    auto iter = temp_tensors_.find(name);
    if (iter != temp_tensors_.end()) return iter->second;
    else throw std::runtime_error(ErrorBase + "There is no internal tensor whose name is " + name);
}
