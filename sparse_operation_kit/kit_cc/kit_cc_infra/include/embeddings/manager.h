...
public:
    ...
    static std::shared_ptr<EmbeddingManager> Create(const std::shared_ptr<ResourcesManager>& resource_mgr);
    ...
    void forward(std::shared_ptr<EmbeddingLayer>& embedding,
...
private:
    ...
    std::vector<std::shared_ptr<HugeCTR::GeneralBuffer2<HugeCTR::CudaAllocator>>> buffers_; // used to save internal spaces for all embedding.
