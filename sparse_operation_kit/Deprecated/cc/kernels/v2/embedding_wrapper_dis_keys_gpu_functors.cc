...
tensorflow::Status EmbeddingWrapper<TypeKey, TypeFP>::distribute_keys_gpu_distributed(
    ...
    auto params = get_item_from_map(embedding_hyper_params_, embedding_name);
    ...
    if (to_csr_events.empty()) return tensorflow::errors::NotFound(__FILE__, ":", __LINE__, " ",
                    "Do not find cudaEvent_t in converting to csr for embedding: ", embedding_name);
    ...
    for (size_t dev_id = 0; dev_id < gpu_count; ++dev_id) {
        ...
        *(input_space->nnz_array_[dev_id]) = *(internel_spaces->cub_host_num_selected_[dev_id]);
        ...
        /*record cudaEvent_t on each stream*/
        WRAPPER_CUDA_CHECK(cudaEventRecord(to_csr_events[dev_id], local_gpu->get_stream()));
