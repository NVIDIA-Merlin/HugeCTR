...
/*This op is used to create a hugectr embedding layer instance.*/
...
public:
    explicit CreateEmbeddingOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
        ...
        OP_REQUIRES(ctx, HugeCTR::find_item_in_map(embedding_type_, embedding_type, HugeCTR::Version2::EMBEDDING_TYPE_MAP),
                    errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                        "Not supported embedding_type."));
        std::string optimizer_type;
        OP_REQUIRES_OK(ctx, ctx->GetAttr("optimizer_type", &optimizer_type));
        OP_REQUIRES(ctx, HugeCTR::find_item_in_map(optimizer_type_, optimizer_type, HugeCTR::Version2::OPTIMIZER_TYPE_MAP),
                    errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                        "Not supported optimizer_type."));
        ...
        OP_REQUIRES(ctx, HugeCTR::find_item_in_map(update_type_, update_type_s, HugeCTR::Version2::UPDATE_TYPE_MAP),
                    errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                        "Not supported update_type."));
        OP_REQUIRES_OK(ctx, ctx->GetAttr("atomic_update", &atomic_update_));
        OP_REQUIRES_OK(ctx, ctx->GetAttr("scaler", &scaler_));
        OP_REQUIRES(ctx, Utils::check_in_set(HugeCTR::Version2::SCALER_SET, scaler_),
                    errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
        ...
        OP_REQUIRES(ctx, HugeCTR::find_item_in_map(combiner_, combiner, HugeCTR::Version2::COMBINER_MAP),
                    errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                        "Not supported combiner."));
    }

    void Compute(OpKernelContext* ctx) override {
        /*create embedding layer*/
        OP_REQUIRES(ctx, HugeCTR::Version2::wrapper, errors::Aborted(__FILE__, ":", __LINE__, " ",
                        "There is no wrapper, hugectr.init() should be called first."));
        OP_REQUIRES_OK(ctx, HugeCTR::Version2::wrapper->create_embedding(name_, embedding_type_, optimizer_type_,
        ...
        /*get init_value for initialization*/
        const Tensor* init_value_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->input("init_value", &init_value_tensor));
        OP_REQUIRES_OK(ctx, HugeCTR::Version2::wrapper->init_embedding_table(name_, init_value_tensor));
...
REGISTER_KERNEL_BUILDER(Name("V2HugectrCreateEmbedding").Device(DEVICE_GPU), 
