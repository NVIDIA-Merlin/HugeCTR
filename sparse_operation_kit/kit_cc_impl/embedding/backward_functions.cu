...
void expand_input_grad(const size_t global_batch_size, const size_t slot_num, 
  ...
  expand_input_grad_kernel<<<grid_size, block_size, 0, stream>>>(global_batch_size, slot_num,
                                                                 embedding_vec_size, replica_row_offset,
                                                                 wgrad, replica_input_grad);  
}
...
void backward_mean(size_t batch_size, size_t slot_size, size_t embedding_vec_size,
    ...
    HugeCTR::backward_mean(batch_size, slot_size, embedding_vec_size, row_offset, top_grad, wgrad, stream);
}

template void backward_mean(size_t batch_size, size_t slot_size, size_t embedding_vec_size,
                   const int64_t *row_offset, const float *top_grad,
                   float *wgrad, cudaStream_t stream);
