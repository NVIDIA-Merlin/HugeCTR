...
public:
    ...
    void forward(const Context_t &replica_context, const bool training) override {
        ...
        CK_NCCL(ncclReduceScatter(embedding_features->GetPtrWithType<float>(),
                                  ...
                                  replica_output->get_num_elements(),
                                  ncclFloat,
                                  ncclSum,
                                  local_gpu->get_nccl(),
        ...
        if (replica_context->has_internal_tensor("row_offset_allreduce_tensor")) {
            auto &row_offset_allreduce_tensor = replica_context->get_internal_tensor("row_offset_allreduce_tensor");
            ...
            const size_t emb_vec_size = replica_output->get_num_elements() / (replica_batch_size * rows_num_per_sample);
