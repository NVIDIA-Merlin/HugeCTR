...
void save_params_helper(const std::shared_ptr<ParamInterface> &param, 
    ...
    for (size_t dev_id = 0; dev_id < local_gpu_count; ++dev_id) {
        ...
        CK_CUDA(cudaMalloc(&d_hash_table_value[dev_id], local_worker_max_count * embedding_vector_size * sizeof(float)));
        ...
        for (size_t worker = 0; worker < worker_num; worker++) {
                    ...
                    if (worker == recv_worker) { /*chief worker receives valid data from other worker*/ 
                        for (size_t dev_id = 0; dev_id < local_gpu_count; dev_id++) {
                            ...
                            CK_NCCL(ncclRecv(d_hash_table_value[dev_id], 
                                            pair_count * embedding_vector_size,
            ...
            for (size_t dev_id = 0; dev_id < local_gpu_count; dev_id++) {
                ...
                CK_CUDA(cudaMemcpyAsync(keys->GetPtrWithType<int64_t>() + host_num_keys_offset, 
                                        d_hash_table_key[dev_id],
                                        pair_count * sizeof(int64_t),
                                        cudaMemcpyDeviceToHost,
                                        local_gpu->get_stream()));
                CK_CUDA(cudaMemcpyAsync(embedding_values->GetPtrWithType<float>() + host_num_keys_offset * embedding_vector_size,
...
void restore_params_helper(std::shared_ptr<ParamInterface> &param,
    ...
    for (size_t i = 0; i < loop_num; i++) {
        ...
        for (size_t id = 0; id < local_gpu_count; id++) {
            ...
            param->get_hashtable(id)->insert(d_hash_table_key_chunk_per_gpu[id], 
                                            value_index_buf, tile_count,
                                            local_gpu->get_stream());
            param->get_hashtable(id)->get_and_add_value_head(tile_count, 
                                local_gpu->get_stream());
        } // for id in local_gpu_count
        ...
        for (size_t id = 0; id < local_gpu_count; id++) {
            ...
            float *dst_buf = param->get_embedding_table_tensor(id)->GetPtrWithType<float>() + value_chunk_offset;
    ...
    for (size_t i = 0; i < remain_loop_num; i++) {
        ...
        if (worker_id == dst_worker) {
            ...
            // hashtable insert
            param->get_hashtable(local_gpu_id)->insert(d_hash_table_key_chunk_per_gpu[local_gpu_id],
                                                        value_index_buf, hash_table_key_tile_size,
                                                        local_gpu->get_stream());
            param->get_hashtable(local_gpu_id)->get_and_add_value_head(
            ...
            value_dst_buf = param->get_embedding_table_tensor(local_gpu_id)->GetPtrWithType<float>() + value_offset;
