...
from sparse_operation_kit.experiment.communication import broadcast
from sparse_operation_kit.experiment.communication import global_gpu_id
...
from sparse_operation_kit.experiment.dynamic_variable import DynamicVariable, export, assign
...
def check_optimizer_is_valid(optimizer, dump_vars):
    """
    The purpose of this function is to check if the optimizer has states for every vars.
    There are 2 cases that can pass, first is if all vars have states in the optimizer,
    second is if all vars don't have states in the optimizer.
    Otherwise will raise a error
    """
    vars_unique_ids = []
...
def load_table(path, load_vars, optimizer):
    if optimizer is not None:
        if (
            ...
            and (
                "sparse_operation_kit.experiment.optimizer.OptimizerWrapper"
                not in str(type(optimizer))
            )
        ):
            raise Exception(
...
def dump(path, dump_vars, optimizer=None):
    """
    Abbreviated as ``sok.experiment.dump``.
    ...
    dump_vars: Can be a single or list of sok.Variable and sok.DynamicVariable
    optimizer:optional,when model train , you want to dump optimizer state,send tf.keras.optimizer or sok.OptimizerWrapper
...
def load(path, load_vars, optimizer=None):
    """
    Abbreviated as ``sok.experiment.load``.
    ...
    ----------
    path: weight file folder
    load_vars: Can be a single or list of sok.Variable and sok.DynamicVariable
    optimizer:optional,when model train , you want to dump optimizer state,send tf.keras.optimizer or sok.OptimizerWrapper
