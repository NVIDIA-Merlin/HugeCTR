...
SparseEmbedding get_sparse_embedding_from_json(const nlohmann::json& j_sparse_embedding) {
  ...
  if (!find_item_in_map(hybrid_embedding_param.hybrid_embedding_type, hybrid_embedding_type_string,
    ...
    CK_THROW_(Error_t::WrongInput, "No such hybrid embedding type: " + hybrid_embedding_type_string);
  }
  ...
  switch (embedding_type) {
    ...
    case Embedding_t::HybridSparseEmbedding: {
      ...
      const HybridSparseEmbeddingParams<TypeFP> embedding_params = {
          ...
          sparse_embedding.hybrid_embedding_param.max_num_frequent_categories * std::max(batch_size, batch_size_eval),  // TBD
...
template void add_sparse_embedding<long long, float>(SparseEmbedding&,
            ...
            std::shared_ptr<ExchangeWgrad>&,
            bool, bool, bool, size_t,
