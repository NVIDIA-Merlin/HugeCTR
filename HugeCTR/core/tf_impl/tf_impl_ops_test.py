...
_tf_impl_ops = tf.load_op_library('./tf_impl_ops_test.so')
...
class TFImplOpsTest(test.TestCase):
  ...
  def test_storage_impl_on_gpu(self):
    ...
    with self.session(use_gpu=gpu_list, config=default_config) as sess:
      ...
      self.assertTrue("on_gpu={}".format(on_gpu) in printed.contents())
      self.assertTrue(
          "allocator=GPU_{}_bfc".format(gpu_id) in printed.contents())

  def test_storage_impl_on_cpu(self):
    with self.session(config=default_config):
      ...
      self.assertTrue("total size={}".format(initial_size +
                                             extend_size) in printed.contents())
      self.assertTrue("cpu_id={}".format(cpu_id) in printed.contents())
      self.assertTrue("on_gpu={}".format(on_gpu) in printed.contents())
      self.assertTrue("allocator=gpu_host_bfc" in printed.contents())
  ...
  def test_tf_backend_impl(self):
    ...
    with self.session(use_gpu=gpu_list, config=default_config) as sess:
      with self.captureWritesToStream(sys.stderr) as printed:
        self.evaluate(_tf_impl_ops.tf_backend_test_op_test())
