...
namespace HugeCTR
{
  namespace gpu_barrier
  {
    ...
    __global__ void sync_all_gpus_cuda(size_t** d_rem_barrier_flags, 
        size_t my_local_id, size_t ndevs, bool enforce_order = false)
    {
      if (enforce_order) { __threadfence_system(); }
  ...
  void GPUBarrier::sync_all_gpus(const cudaStream_t stream, size_t device_id)
  {
    ...
    CK_CUDA_THROW_(cudaSetDevice(dev_list_[device_id]));
      sync_all_gpus_cuda<<<n_blocks, MAX_TPB, 0, stream>>>(
          d_rem_barrier_flags_[device_id], device_id, num_gpus_,
          enforce_order_);
  }
      ...
      size_t* h_report_ptr, const cudaStream_t stream, size_t device_id)
  {
    ...
    sync_all_gpus_report_host_and_inc_cuda<<<1, num_gpus_, 0, stream>>>(
        d_rem_barrier_flags_[device_id], d_report_count, h_report_ptr, device_id, num_gpus_, enforce_order_);
  }
