...
template <typename T>
__global__ void sgd_update_kernel(int len, float* weight, const T* wgrad, const float* lr_ptr,
                                  float scaler) {
...
void SGDOptimizer<T>::update() {
  ...
  PROFILE_RECORD("update.start", gpu_resource_->get_stream());
  ...
  if (gpu_learning_rate_scheduler_ == nullptr) {
    sgd_update_kernel<<<grid_dim, block_dim, 0, gpu_resource_->get_stream()>>>(len, weight, wgrad,
                                                                               lr_, scaler_);
  } else {
  ...
  PROFILE_RECORD("update.stop", gpu_resource_->get_stream());
#ifndef NDEBUG
