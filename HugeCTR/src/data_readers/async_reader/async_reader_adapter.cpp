...
AsyncReader<SparseType>::AsyncReader(
  std::string fname, size_t batch_size, size_t label_dim, size_t dense_dim,
  ...
  for (uint32_t i = 0; i < resource_manager_->get_local_gpu_count(); i++) {
    ...
    if (mixed_precision_) {
      ...
      auto dense_buffer_ = std::make_shared<RawPtrBuffer>(batch_size * dense_dim_align8 * sizeof(float));
      dense_tensors_.emplace_back(
...
void AsyncReader<SparseType>::schedule_here_graph(cudaStream_t stream, int raw_device_id) {
  CK_CUDA_THROW_( cudaEventRecordWithFlags(
          schedule_events_[raw_device_id], stream, cudaEventRecordExternal) );
}
...
template <typename SparseType> 
void AsyncReader<SparseType>::create_drwg_norm(
