...
namespace HugeCTR {
  ...
  __device__ __forceinline__ unsigned int abs(unsigned int x) {
    return x;
  }
  ...
  __global__ void cat_local_slot_converter_kernel__(int64_t *cat_data_column_ptrs, int num_params,
                                                  int param_id, int num_slots,
                                                  int batch_size, int num_devices,
                                                  int32_t *dev_slot_per_device,
    ...
    uint32_t *block_atomic_accum = reinterpret_cast<uint32_t*>(smem_ +
                                                              (num_warps * warpSize *
                                                                smem_pitch * sizeof(T)));
  ...
  template <typename T>
  __global__ void cat_distributed_slot_csr_val_kernel__(int64_t *cat_data_column_ptrs,
                                                int num_params, int param_id, int num_slots,
                                                  int batch_size, int num_devices,
                                                  T* dev_slot_offset_ptr,
...
*/
  ...
  void convert_parquet_dense_columns(std::vector<T*>& dense_column_data_ptr,
    ...
    int64_t *pinned_dev_out_buffer = reinterpret_cast<int64_t*>((size_t)(dev_ptr_staging) +
                                                                           size_of_col_ptrs);
    ...
    dense_data_converter_kernel__<T><<<grid, block, 0, task_stream>>>(
                                (int64_t*)dev_in_column_ptr.data(), label_dense_dim,
                                batch_size, num_dense_buffers, (int64_t*)dev_out_data_ptr.data() );
...
*/
  ...
  size_t convert_parquet_cat_columns(std::vector<T*>& cat_column_data_ptr,
    ...
    int64_t *pinned_csr_val_out_buffer = reinterpret_cast<int64_t*>((size_t)(dev_ptr_staging) +
                                                                               size_of_col_ptrs);
    ...
    int64_t *pinned_csr_row_offset_buffer = reinterpret_cast<int64_t*>(
                                                        (size_t)(dev_ptr_staging) +
                                                        size_of_col_ptrs + size_of_csr_pointers);
    for (int i = 0; i < num_csr_buffers; i++) {
    ...
    if (distributed_slot) {
      ...
      for (int i = 0; i < num_csr_buffers; i++) {
        ...
        CK_CUDA_THROW_(cudaMemsetAsync(csr_row_offset_staging.back().data(),
                                        0, csr_roff_buf_size, task_stream));
      }

      int64_t* pinned_csr_row_offset_staging = reinterpret_cast<int64_t*>(
                                                (size_t)pinned_csr_row_offset_buffer
                                                  + size_of_csr_pointers);
      ...
      cat_distributed_slot_csr_roffset_kernel__<T><<<grid, block, smem_size, task_stream>>>(
                                                  (int64_t*)dev_in_column_ptr.data(), num_params,
                                                  param_id, num_slots, batch_size, num_devices,
                                                  dev_slot_offset_ptr,
                                                  (int64_t *)dev_csr_row_offset_staging_ptr.data(),
      ...
      CK_CUDA_THROW_(cub::DeviceScan::ExclusiveSum(tmp_storage, temp_storage_bytes,
                                            reinterpret_cast<T*>(pinned_csr_row_offset_staging[0]),
                                            reinterpret_cast<T*>(pinned_csr_row_offset_buffer[0]),
                                            prefix_sum_items, task_stream));
      ...
      for (int i = 0; i < num_devices; i++) {
        ...
        CK_CUDA_THROW_(cub::DeviceScan::ExclusiveSum(cub_tmp_storage.data(), temp_storage_bytes,
      ...
      cat_distributed_slot_csr_val_kernel__<T><<<grid, block, smem_size, task_stream>>>(
                                                ...
                                                param_id, num_slots, batch_size, num_devices,
                                                dev_slot_offset_ptr,
                                                (int64_t *)dev_csr_value_ptr.data(),
                                                (int64_t *)dev_csr_row_offset_ptr.data());
      CK_CUDA_THROW_(cudaGetLastError());
    } else {

      int32_t *pinned_slot_per_device = reinterpret_cast<int32_t*>(
                                      (size_t)pinned_csr_row_offset_buffer + size_of_csr_pointers);
      // localized embedding , generate slot to idx count mappings
      for (int i = 0; i < num_devices; i++)
        pinned_slot_per_device[i] = 0;
      ...
      cat_local_slot_converter_kernel__<T><<<grid, block, smem_size, task_stream>>>(
                                                    (int64_t*)dev_in_column_ptr.data(), num_params,
                                                    param_id, num_slots, batch_size, num_devices,
                                                    (int32_t *)dev_slot_per_device_ptr.data(),
                                                    dev_slot_offset_ptr,
                                                    (int64_t *)dev_csr_value_ptr.data(),
                                                    (int64_t *)dev_csr_row_offset_ptr.data(),
                                                    dev_embed_param_offset_buf );
      ...
      // future - take-in atomic_offset_counter from last fn call to start at right offset of csr_row_offset_buf
      // same offset goes to converter_kernel as well for both value, row_offset buffer
...
template void convert_parquet_dense_columns<float>(
  std::vector<float*>& dense_column_data_ptr,
  const int label_dense_dim, 
  int batch_size, int num_dense_buffers,
  std::vector<rmm::device_buffer>& dense_data_buffers,
  int64_t *dev_ptr_staging,
...
template size_t convert_parquet_cat_columns<unsigned int>(
  ...
  int64_t *dev_ptr_staging,
  uint32_t *dev_embed_param_offset_buf,
  unsigned int *dev_slot_offset_ptr,
  std::deque<rmm::device_buffer>& rmm_resources,
  rmm::mr::device_memory_resource *mr,
  cudaStream_t task_stream);
