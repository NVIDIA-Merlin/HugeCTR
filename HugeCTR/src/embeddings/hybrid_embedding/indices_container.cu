...
IndexProcessor<dtype>::IndexProcessor(
    ...
    size_t queue_size, size_t batch_size,
  ...
  for (size_t qid=0; qid<queue_size; qid++) {
    ...
    for (size_t raw_device_id = 0; raw_device_id < local_gpu_count; raw_device_id++) {
      ...
      container.infrequent_selections.emplace_back(container.datas[raw_device_id], models[raw_device_id]);
      ...
      CK_CUDA_THROW_(cudaMemset(dense_tensor_bag.get_ptr(), 0,
                                dense_tensor_bag.get_size_in_bytes()));
    }
  }
}

template<typename dtype>
void IndexProcessor<dtype>::calculate_indices(BatchDesc batch, size_t queue_id, int raw_device_id, cudaStream_t stream)
{
    auto& local_gpu = resource_manager_->get_local_gpu(raw_device_id);
    ...
    compute_indices(container.frequent_compressions[raw_device_id],
                    container.infrequent_selections[raw_device_id],
                    communication_type_, true,
                    stream,
                    local_gpu->get_sm_count());
}

template<typename dtype>
void IndexProcessor<dtype>::split3way(BatchDesc batch, size_t queue_id, int raw_device_id, cudaStream_t stream)
{
  auto global_dev_id = resource_manager_->get_gpu_global_id_from_local_id(raw_device_id);
  auto& container = containers_[queue_id];
  auto& my_data = container.datas[raw_device_id];
  auto ptr_wrap = std::make_shared<RawPtrWrapper>(reinterpret_cast<InputType*>(batch.dev_data[raw_device_id]));
  ...
  if (mixed_precision_) {
    split_3_way<__half, SparseType>(
            ...
            Tensor2<__half>   ::stretch_from(container.dense_tensors[raw_device_id]),
            in_place_tensor,
    ...
    split_3_way<float, SparseType>(
            Tensor2<LabelType>::stretch_from(container.label_tensors[raw_device_id]),
            Tensor2<float>    ::stretch_from(container.dense_tensors[raw_device_id]),
            in_place_tensor,
            Tensor2<InputType>({batch_size_, sample_size_items_}, ptr_wrap),
            global_dev_id * batch_size_per_dev_, (global_dev_id+1) * batch_size_per_dev_,
            stream);
...
template<typename dtype>
void IndexProcessor<dtype>::finalize(
    TensorBag2& label_tensor, TensorBag2& dense_tensor, SparseTensorBag& sparse_tensor,
    size_t queue_id, int raw_device_id, cudaStream_t stream)
{
    auto& container = containers_[queue_id];

    if ( (char*)label_tensor.get_ptr() + label_tensor.get_size_in_bytes() == (char*)dense_tensor.get_ptr() ) {
      CK_CUDA_THROW_(cudaMemcpyAsync(
              label_tensor.get_ptr(),
              container.label_tensors[raw_device_id].get_ptr(),
      ...
      CK_CUDA_THROW_(cudaMemcpyAsync(
              label_tensor.get_ptr(),
              container.label_tensors[raw_device_id].get_ptr(),
              label_tensor.get_size_in_bytes(),
              cudaMemcpyDeviceToDevice,
              stream));

      CK_CUDA_THROW_(cudaMemcpyAsync(
              dense_tensor.get_ptr(),
              container.dense_tensors[raw_device_id].get_ptr(),
              dense_tensor.get_size_in_bytes(),
              cudaMemcpyDeviceToDevice,
              stream));
    }
