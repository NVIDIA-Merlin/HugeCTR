...
void SparseEmbeddingFunctors::init_embedding_per_slot(size_t slot_id, size_t slot_size,
  ...
  std::shared_ptr<GeneralBuffer2<CudaAllocator>> dbuff = GeneralBuffer2<CudaAllocator>::create();
  std::shared_ptr<GeneralBuffer2<CudaHostAllocator>> hbuff =
      GeneralBuffer2<CudaHostAllocator>::create();

  Tensor2<TypeHashKey> hash_keys;
  dbuff->reserve({slot_size}, &hash_keys);
  Tensor2<size_t> hash_value_indices;
  dbuff->reserve({slot_size}, &hash_value_indices);

  Tensor2<float> embedding_init;
  hbuff->reserve({slot_size * embedding_vec_size}, &embedding_init);
  ...
  float *embedding_init_ptr = embedding_init.get_ptr();
  for (size_t i = 0; i < (slot_size * embedding_vec_size); i++) {
    embedding_init_ptr[i] = fdata_sim.get_num();
  }
  CK_CUDA_THROW_(cudaMemcpyAsync(embedding_table, embedding_init.get_ptr(),
                                 slot_size * embedding_vec_size * sizeof(float),
                                 cudaMemcpyHostToDevice, stream));

  memset_liner(hash_keys.get_ptr(), (TypeHashKey)key_offset, (TypeHashKey)1, slot_size, stream);
  memset_liner(hash_value_indices.get_ptr(), value_index_offset, 1ul, slot_size, stream);
  hash_table.insert(hash_keys.get_ptr(), hash_value_indices.get_ptr(), slot_size, stream);
  hash_table.get_and_add_value_head(slot_size, stream);
...
void SparseEmbeddingFunctors::init_embedding_per_gpu(size_t lid, size_t gid, size_t total_gpu_count,
  ...
  float *embedding_table_ptr = embedding_table.get_ptr();
  ...
  for (size_t i = 0; i < slot_sizes.size(); i++) {
    ...
    if ((i % total_gpu_count) == gid) {
      ...
      init_embedding_per_slot(i, slot_size, embedding_vec_size, key_offset, value_index_offset,
                              embedding_table_ptr, hash_table, slot_ids_ptr,
                              device_resources[lid].get_stream());
      value_index_offset += slot_size;
      embedding_table_ptr += slot_size * embedding_vec_size;
  ...
  for (size_t i = 0; i < slot_sizes.size(); i++) {
    ...
    if ((i % total_gpu_count) == gid) {
      ...
      init_embedding_per_slot<TypeHashKey>(i, slot_size, embedding_vec_size, key_offset,
                                           value_index_offset, embedding_table_ptr, slot_ids_ptr,
                                           device_resources[lid].get_stream());
      value_index_offset += slot_size;
      embedding_table_ptr += slot_size * embedding_vec_size;
      slot_ids_ptr += slot_size;
...
template void SparseEmbeddingFunctors::init_embedding_per_gpu<unsigned int>(
    ...
    size_t embedding_vec_size, Tensor2<float> &embedding_table,
    HashTable<unsigned int, size_t> &hash_table, Tensor2<size_t> &slot_ids,
    const GPUResourceGroup &device_resources);

template void SparseEmbeddingFunctors::init_embedding_per_gpu<long long>(
    size_t lid, size_t gid, size_t total_gpu_count, const std::vector<size_t> &slot_sizes,
    size_t embedding_vec_size, Tensor2<float> &embedding_table,
    HashTable<long long, size_t> &hash_table, Tensor2<size_t> &slot_ids,
    const GPUResourceGroup &device_resources);

template void SparseEmbeddingFunctors::init_embedding_per_gpu<unsigned int>(
    size_t lid, size_t gid, size_t total_gpu_count, const std::vector<size_t> &slot_sizes,
    size_t embedding_vec_size, Tensor2<float> &embedding_table, Tensor2<size_t> &slot_ids,
    const GPUResourceGroup &device_resources);
