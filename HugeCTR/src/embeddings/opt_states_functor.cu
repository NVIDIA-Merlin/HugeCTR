...
void SparseEmbeddingFunctors::dump_opt_states(
  ...
  for (auto& opt_state: opt_states) {
    ...
    CK_MPI_THROW_(MPI_Reduce(is_master_process? MPI_IN_PLACE : &max_size,
                             &max_size, sizeof(size_t), MPI_CHAR, MPI_MAX,
    ...
    if (resource_manager.is_master_process()) {
      MESSAGE_("Rank" + std::to_string(pid) + ": Write optimzer state to file",
               true);
      stream.write(h_opt_state.get(), total_size);
    }
#ifdef ENABLE_MPI
    else {
      MESSAGE_("Rank" + std::to_string(pid) + ": Send optimzer state to master node",
               true);
      int tag = (pid << 8) | 0xBA;
      CK_MPI_THROW_(MPI_Send(h_opt_state.get(), total_size, MPI_CHAR,
                             resource_manager.get_master_process_id(), tag,
                             MPI_COMM_WORLD));
    }

    if (resource_manager.is_master_process()) {
      for (int r = 1; r < resource_manager.get_num_process(); r++) {
        MESSAGE_("Rank" + std::to_string(pid) +
                 ": Recv optimzer state from rank" + std::to_string(r) +
                 ", and write to file",
                 true);
        int tag = (r << 8) | 0xBA;
        int recv_size = 0;
        ...
        CK_MPI_THROW_(MPI_Recv(h_opt_state.get(), recv_size, MPI_CHAR, r, tag,
  ...
  for (auto& opt_state: opt_states) {
    ...
    auto h2d_op = [&opt_state, &resource_manager, &context](char* h_opt_state) {
      ...
      for (size_t id = 0; id < resource_manager.get_local_gpu_count(); id++) {
        ...
        auto& local_gpu = resource_manager.get_local_gpu(id);
        context.set_device(local_gpu->get_device_id());
        CK_CUDA_THROW_(cudaMemcpyAsync(opt_state[id].get_ptr(), h_opt_state + offset,
                                       local_size,
    ...
    if (resource_manager.is_master_process()) {
      ...
      MESSAGE_("Rank" + std::to_string(pid) + ": Read optimzer state from file",
					     true);
      ...
      MESSAGE_("Rank" + std::to_string(pid) +
               ": Recv optimzer state from master node" +
               ", and write to GPUs",
							 true);
      ...
      CK_MPI_THROW_(MPI_Recv(h_opt_state.get(), recv_size, MPI_CHAR, mid, tag,
                             MPI_COMM_WORLD, MPI_STATUS_IGNORE));
