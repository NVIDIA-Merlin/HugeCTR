...
template void SparseEmbeddingFunctors::all2all_backward<float>(
    ...
    const TensorPtrs<float> &send_tensors, TensorPtrs<float> &recv_tensors,
    const GPUResourceGroup &device_resources);

template void SparseEmbeddingFunctors::all2all_backward<__half>(
    size_t batch_size_per_gpu, size_t slot_num, size_t embedding_vec_size,
    const TensorPtrs<__half> &send_tensors, TensorPtrs<__half> &recv_tensors,
    const GPUResourceGroup &device_resources);
...
template void SparseEmbeddingFunctors::all2all_backward<float>(
    ...
    TensorPtrs<float> &recv_tensors, const GPUResourceGroup &device_resources);

template void SparseEmbeddingFunctors::all2all_backward<__half>(
    size_t batch_size_per_gpu, const std::vector<size_t> &slot_num_per_gpu,
    size_t embedding_vec_size, const TensorPtrs<__half> &send_tensors,
    TensorPtrs<__half> &recv_tensors, const GPUResourceGroup &device_resources);
