...
__global__ void concat_kernel(bool forward, T* out, const int h, const int out_w, const Args... args) {
  ...
  const ConcatLayer2::InParam<T> in_params[] = {args...};
...
ConcatLayer2::ConcatLayer2(std::vector<Tensor<float>*>& in_tensors, Tensor<float>** out_tensor,
...
void ConcatLayer2::fprop(cudaStream_t stream) {
  prop_common(true, stream);
}

void ConcatLayer2::bprop(cudaStream_t stream) {
  prop_common(false, stream);
}

void ConcatLayer2::prop_common(bool forward, cudaStream_t stream) {
  int o_device = -1;
  CK_CUDA_THROW_(get_set_device(get_device_id(), &o_device));
...
std::vector<ConcatLayer2::InParam<float>> ConcatLayer2::set_in_params(int n) {
...
void ConcatLayer2::kernel_launch(bool forward, cudaStream_t stream, Args&... args) {
  int block_size = 256;
