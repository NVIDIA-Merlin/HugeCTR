...
Network::Network(int device_id, const std::shared_ptr<const GPUResource>& gpu_resource,
    ...
    : blobs_buff_(new GeneralBuffer<float>()),
      ...
      eval_graph_created_(false), train_graph_created_(false) {
...
void Network::train() {
  ...
  if (!train_graph_created_) {
    ...
    CK_CUDA_THROW_(cudaStreamEndCapture(gpu_resource_->get_stream(), &train_graph_));
    CK_CUDA_THROW_(cudaGraphInstantiate(&train_instance_, train_graph_, NULL, NULL, 0));
    train_graph_created_ = true;
  }
  CK_CUDA_THROW_(cudaGraphLaunch(train_instance_, gpu_resource_->get_stream()));
