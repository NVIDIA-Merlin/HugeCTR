...
SolverParser::SolverParser(const std::string& file) {
  try {
    ...
    if (!find_item_in_map(lr_policy, lr_policy_string, LR_POLICY)) {
      HCTR_OWN_THROW(Error_t::WrongInput, "No such poliicy: " + lr_policy_string);
    }
    ...
    if (has_max_iter && has_num_epochs) {
      HCTR_OWN_THROW(Error_t::WrongInput, "max_iter and num_epochs cannot be used together.");
    } else {
      if (has_max_iter) {
    ...
    if (has_max_eval_batches && has_max_eval_samples) {
      ...
      if (has_max_eval_batches) {
        ...
        if (rem) {
          HCTR_LOG_S(INFO, ROOT) << "max_eval_samples(" << max_eval_samples
                                 << ") is not divisible by batchsize_eval(" << batchsize_eval
                                 << ". The remainder is truncated." << std::endl;
        }
    ...
    if (has_key_(j, "mixed_precision") && has_key_(j, "enable_tf32_compute")) {
      HCTR_OWN_THROW(Error_t::WrongInput,
                     "mixed_precision and enable_tf32_compute MUST not coexist");
    }
    ...
    if (has_key_(j, "mixed_precision")) {
      ...
      scaler = i_scaler;
      HCTR_LOG_S(INFO, ROOT) << "Mixed Precision training with scaler: " << i_scaler
                             << " is enabled." << std::endl;
    } else {
    ...
    HCTR_LOG_S(INFO, ROOT) << "TF32 Compute: " << (enable_tf32_compute ? "ON" : "OFF") << std::endl;
    ...
    if (gpu_array[0].is_array()) {
      for (auto gpu : gpu_array) {
        ...
        for (auto gpu_tmp : gpu) {
          ...
          if (gpu_id < 0) {
            HCTR_OWN_THROW(Error_t::WrongInput, "gpu_id < 0");
          }
        }
    ...
    if (has_key_(j, "device_layout")) {
      ...
      if (device_layout_str == "LocalFirst") {
        ...
        HCTR_OWN_THROW(Error_t::WrongInput,
                       "Invalid device layout. Options are: LocalFirst, NodeFirst");
      }
    } else {
      device_layout = DeviceMap::LOCAL_FIRST;
    }
    ...
    if (has_key_(j, "eval_metrics")) {
      ...
      if (eval_metrics.is_array()) {
        for (auto metric : eval_metrics) {
          std::istringstream is(metric.get<std::string>());
          std::string elem;
          ...
          if (it != metrics_map.end()) {
            ...
            switch (type) {
              ...
              case metrics::Type::AUC: {
                ...
                if (val < 0.0 || val > 1.0) {
                  HCTR_OWN_THROW(Error_t::WrongInput, "0 <= AUC threshold <= 1 is not true");
                }
              ...
              default: {
                HCTR_OWN_THROW(Error_t::WrongInput, "Unreachable");
                break;
              }
            }
          } else {
            HCTR_OWN_THROW(Error_t::WrongInput, metric_strs[0] + " is a unsupported metric");
          }
        }
      } else {
        HCTR_OWN_THROW(Error_t::WrongInput, "metrics must be in the form of list");
      }
    } else {
      // Default is AUC without the threshold
      HCTR_LOG(INFO, ROOT, "Default evaluation metric is AUC without threshold value\n");
    ...
    if (has_key_(j, "input_key_type")) {
      ...
      if (str.compare("I64") == 0) {
        ...
        HCTR_OWN_THROW(Error_t::WrongInput, "input_key_type is I64 or I32");
      }
    } else {
      i64_input_key = false;
    }
    ...
    if (has_key_(j, "export_predictions_prefix")) {
      ...
      HCTR_LOG_S(INFO, ROOT) << "Export prediction prefix: " << export_predictions_prefix
                             << std::endl;
    } else {
      export_predictions_prefix = "";
      HCTR_LOG(INFO, ROOT, "Export prediction: OFF\n");
    }

    async_mlp_wgrad = get_value_from_json_soft<bool>(j, "async_mlp_wgrad", true);
    HCTR_LOG_S(INFO, ROOT) << "Asynchronous Wgrad computation of MLP: "
                           << (async_mlp_wgrad ? "ON" : "OFF") << std::endl;
