...
__global__ void decompress_emb_vec(
  const float* d_src_emb_vec, 
  const uint64_t* d_src_index, 
  float* d_dst_emb_vec, 
  const size_t len, 
  const size_t emb_vec_size)
{
  ...
  if( idx < (len * emb_vec_size) )
  {
    ...
    d_dst_emb_vec[dst_emb_vec * emb_vec_size + dst_float] = d_src_emb_vec[src_emb_vec * emb_vec_size + dst_float];
  }
}
...
void decompress_emb_vec_async(const float* d_unique_src_ptr, 
  ...
  decompress_emb_vec<<<((decompress_len_in_float - 1) / BLOCK_SIZE) + 1, BLOCK_SIZE, 0, stream>>>(d_unique_src_ptr, 
                                                                                                  d_unique_index_ptr, 
