...
void DPModelForward::compute(const Tensor &bucket_range, const Tensor &d_combiner_list,
                             const TensorList &dp_ev, const Tensor &dp_offset, const Tensor &dp_dst,
  ...
  DISPATCH_INTEGRAL_FUNCTION(bucket_range.dtype().type(), offset_t, [&] {
    DISPATCH_FLOAT_AND_HALF_FUNCTION(output_buffer.dtype().type(), emb_t, [&] {
      ...
      auto multi_to_one_desc = make_MultiToOne<float, emb_t>(
          ...
          [=] __device__(int i) { return dp_offset_ptr[i]; },
          [=] __device__(int i) {
            int i_dst_ev = dp_dst_ptr[i];
            int lookup_id = i_dst_ev / batch_size_per_gpu;
            int bid = i_dst_ev % batch_size_per_gpu;
