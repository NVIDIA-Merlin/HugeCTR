...
class EmbeddingPlanner {
 ...
 public:
  ...
  const std::vector<std::vector<EmbeddingShardingParam>>
      &get_gpu_major_global_embedding_sharding_param_list() const {
    return global_embedding_sharding_param_list_;
  }

  std::vector<std::vector<EmbeddingShardingParam>>
  get_table_major_global_embedding_sharding_param_list() const {
    ...
    for (size_t table_id = 0; table_id < num_table; ++table_id) {
      for (size_t gpu_id = 0; gpu_id < global_embedding_sharding_param_list_.size(); ++gpu_id) {
        table_major_param_list[table_id].push_back(
            global_embedding_sharding_param_list_[gpu_id][table_id]);
      }
    }
  ...
  std::unique_ptr<IEmbeddingCollectionForward> create_embedding_collection_forward(
      std::shared_ptr<CoreResourceManager> core);

  std::unique_ptr<IEmbeddingCollectionBackward> create_embedding_collection_backward(
      std::shared_ptr<CoreResourceManager> core);
};
}  // namespace embedding
