 ...
 * @param p_loop_flag a flag to control the loop,
          and break loop when DataReaderWorkerEx is destroyed.
 */
template <typename TypeKey>
static void data_reader_thread_func_(const std::shared_ptr<DataReaderWorkerEx<TypeKey>>& data_reader,
                                     int* p_loop_flag) {
  try {
    while (*p_loop_flag) {
 ...
 * CSRChunkEx to data_reader (GPU) local buffer in a new thread.
 ...
 * An instance of DataReaderEx will maintain independent
 * threads for data reading (DataReaderWorkerEx)
...
class DataReaderEx {
 private:
  ...
  std::shared_ptr<Heap<CSRChunkEx<TypeKey>>> csr_heap_; /**< heap to cache the data set */
  ...
  /**
   * Ctor.
   * This ctor is only used when you already have a instant of DataReaderEx and you want to
   * reuse the output tensor e.g. evaluation.
   * @params file_list_name file name of the data set file list.
   * @params prototype an instant of crated DataReaderEx for output reuse.
   * @params num_chunks number of chunks in heap.
   * @params number of threads for data reading.
   */
  DataReaderEx(const std::string& file_list_name, const DataReaderEx& prototype, int num_chunks = 31,
             int num_threads = 20);
   ...
   */
  DataReaderEx(const DataReaderEx& prototype);
 ...
 public:
   ...
   * Ctor
   */
  DataReaderEx(const std::string& file_list_name, int batchsize, int label_dim, int dense_dim,
	       Check_t check_type, std::vector<DataReaderSparseParam>& params,
	       const std::shared_ptr<GPUResourceGroup>& gpu_resource_group,
	       int num_chunks = 31, int num_threads = 20);

  /**
   * Slave process of evaluation will call this to create a new object of DataReaderEx
   */
  DataReaderEx* clone_eval_with_shared_output() {
    DataReaderEx* new_reader = new DataReaderEx(*this);
    return new_reader;
  }
   ...
   * Slave process of evaluation will call this to create a new object of DataReaderEx
   */
  DataReaderEx* clone_eval_with_shared_output(const std::string& file_list_name) {
    DataReaderEx* new_reader = new DataReaderEx(file_list_name, *this);
    return new_reader;
  }
...
template <typename TypeKey>
DataReaderEx<TypeKey>::DataReaderEx(const std::string& file_list_name,
                                const DataReaderEx<TypeKey>& prototype, int num_chunks,
                                int num_threads)
...
template <typename TypeKey>
DataReaderEx<TypeKey>::DataReaderEx(const DataReaderEx<TypeKey>& prototype)
    : label_dense_buffers_(prototype.label_dense_buffers_),
      label_tensors_(prototype.label_tensors_),
      dense_tensors_(prototype.dense_tensor_),
      check_type_(prototype.check_type),
...
template <typename TypeKey>
DataReaderEx<TypeKey>::DataReaderEx(const std::string& file_list_name, int batchsize, int label_dim, int dense_dim,
				Check_t check_type, std::vector<DataReaderSparseParam>& params,
                                const std::shared_ptr<GPUResourceGroup>& gpu_resource_group,
...
template <typename TypeKey>
void DataReaderEx<TypeKey>::read_a_batch_to_device() {
