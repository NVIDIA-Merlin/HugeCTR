...
class Session {
 ...
 public:
   ...
   */
  Error_t load_params(const std::string& model_file, const std::vector<std::string>& embedding_file);
   ...
   * @param device_map a index list of the devices which be used in this training.
   */
  Session(int batch_size, const std::string& model_file, const std::vector<std::string>& embedding_files,
          const std::string& json_name, const std::shared_ptr<const DeviceMap>& device_map)
      : Session(batch_size, json_name, device_map) {
    load_params(model_file, embedding_files);
  }
  /**
   * Dtor of Session.
   */
