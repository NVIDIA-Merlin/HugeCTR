...
class Session {
 private:
  ...
  std::shared_ptr<GPUResourceGroup> gpu_resource_group_; /**< GPU resources include handles and streams etc.*/

  Error_t download_params_to_files_(std::string weights_file, const std::vector<std::string>& embedding_files);
   ...
   */
  Error_t load_params_(const std::string& model_file, const std::vector<std::string>& embedding_file);
 public:


  Session(const std::string& json_name);
  
  const SolverParser get_solver_config() const{
