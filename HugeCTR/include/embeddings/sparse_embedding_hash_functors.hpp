...
class SparseEmbeddingHashFunctors {
 ...
 public:
  ...
  void forward(
    ...
    for (int id = 0; id < local_gpu_count; id++) {
      ...
      try {
        ...
        size_t num;
        CK_CUDA_THROW_(cudaMemcpyAsync(&num, &row_offset[batch_size * slot_num],
  ...
  void backward(int batch_size, int slot_num, int embedding_vec_size, int combiner,
                const Tensors<TypeHashKey> &row_offset_allreduce_tensors,
                const Tensors<float> &embedding_feature_tensors, Tensors<float> &wgrad_tensors,
                const std::shared_ptr<GPUResourceGroup> &device_resources,
                const CudaDeviceContext &context) {
    int local_gpu_count = device_resources->size();
  ...
  void update_params(cudaStream_t stream, int batch_size, int slot_num, int embedding_vec_size,
                     size_t max_vocabulary_size_per_gpu, OptParams &opt_params,
                     ...
                     TypeHashValueIndex *hash_value_index, TypeHashKey *sample_id,
                     TypeHashKey *sample_id_sort, TypeHashValueIndex *hash_value_index_sort,
                     ...
                     float *hash_table_value, float scaler) {
    if (slot_num == 0) {
      return;
    }
        ...
        switch (opt_params.optimizer) {
          case 0:  // adam
            ...
            opt_adam_kernel<<<gridSize, blockSize, 0, stream>>>(
                hash_hash_value_index_count_num, embedding_vec_size, opt_params.hyperparams.adam,
                sample_id_sort, hash_value_index_sort, hash_value_index_count_offset, wgrad,
                deltaw_hash_value_index, (float *)deltaw, scaler);
            break;
          case 1:  // momentum sgd
   ...
   * @param context gpu device context, for switching device.
   */
  void reduce_scatter(int recv_count, const Tensors<float> &send_tensors,
   ...
   */
  template <typename TypeHashKey>
  void all_reduce(int send_count, const Tensors<TypeHashKey> &send_tensors,
   ...
   * @param context gpu device context, for switching device.
   */
  void all_gather(int send_count, const Tensors<float> &send_tensors, Tensors<float> &recv_tensors,
                  const std::shared_ptr<GPUResourceGroup> &device_resources,
                  const CudaDeviceContext &context) {
    int local_gpu_count = device_resources->size();
    int total_gpu_count = device_resources->get_total_gpu_count();
    ...
    // for single GPU, just do memcpyD2D
      ...
      context.set_device((*device_resources)[0]->get_device_id());
      CK_CUDA_THROW_(cudaMemcpyAsync(recv_tensors[0]->get_ptr(), send_tensors[0]->get_ptr(),
                                     send_count * sizeof(float), cudaMemcpyDeviceToDevice,
                                     (*device_resources)[0]->get_stream()));
    }
  }
  ...
  void all2all_forward(int batch_size_per_gpu, int slot_num, int embedding_vec_size,
...
#ifndef NDEBUG
    std::cout << "my_rank=" << my_rank << ", forward nccl all2all send_table:" << std::endl;
    for (int i = 0; i < local_gpu_count; i++) {
      for (int j = 0; j < total_gpu_count; j++) {
    ...
    std::cout << "my_rank=" << my_rank << ", forward nccl all2all recv_table:" << std::endl;
    for (int i = 0; i < local_gpu_count; i++) {
      for (int j = 0; j < total_gpu_count; j++) {
  ...
  void all2all_backward(int batch_size_per_gpu, const int slot_num, int embedding_vec_size,
...
#ifndef NDEBUG
    std::cout << "my_rank=" << my_rank << ", backward nccl all2all send_table:" << std::endl;
    for (int i = 0; i < local_gpu_count; i++) {
      for (int j = 0; j < total_gpu_count; j++) {
        std::cout << send_table[i][j] << ", ";
      }
    ...
    std::cout << "my_rank=" << my_rank << ", backward nccl all2all recv_table:" << std::endl;
    for (int i = 0; i < local_gpu_count; i++) {
      for (int j = 0; j < total_gpu_count; j++) {
        std::cout << recv_table[i][j] << ", ";
      }
   ...
   * @param device_resources all gpus device resources.
   */
  void all2all_init_forward(std::unique_ptr<comm_handler> &all2all, const std::string &plan_file,
                            int batch_size_per_gpu, const std::vector<int> &slot_num_per_gpu,
                            int embedding_vec_size, const Tensors<float> &send_tensors,
                            Tensors<float> &recv_tensors,
                            const std::shared_ptr<GPUResourceGroup> &device_resources) {
    ...
    all2all = std::unique_ptr<comm_handler>(
        new comm_handler(plan_file, device_ids));  // The all2all communication class
   ...
   * @param device_resources all gpus device resources.
   */
  void all2all_init_backward(std::unique_ptr<comm_handler> &all2all, const std::string &plan_file,
                             int batch_size_per_gpu, const std::vector<int> &slot_num_per_gpu,
                             int embedding_vec_size, const Tensors<float> &send_tensors,
                             Tensors<float> &recv_tensors,
                             const std::shared_ptr<GPUResourceGroup> &device_resources) {
   ...
   */
  void all2all_init_forward(std::unique_ptr<comm_handler> &all2all, const std::string &plan_file,
                            int batch_size_per_gpu, int slot_num, int embedding_vec_size,
                            const Tensors<float> &send_tensors, Tensors<float> &recv_tensors,
                            const std::shared_ptr<GPUResourceGroup> &device_resources) {
    using transfer_plan_t = comm_handler_traits::transfer_plan_t;
    transfer_plan_t *transfer_plan = new transfer_plan_t(parse_plan(plan_file.c_str()));
  ...
  void forward_reorder(int batch_size_per_gpu, int slot_num, int embedding_vec_size,
                       Tensors<float> &src_tensors, Tensors<float> &dst_tensors,
                       const std::shared_ptr<GPUResourceGroup> &device_resources,
                       const CudaDeviceContext &context) {
    int local_gpu_count = device_resources->size();
    int total_gpu_count = device_resources->get_total_gpu_count();
  ...
  void memset_liner(cudaStream_t stream, Type *data, Type start_value, Type stride_value,
                    size_t n) {
    try {
      int blockSize = 256;
  ...
  void upload_params_to_device(
      ...
      std::vector<std::unique_ptr<HashTable<TypeHashKey, TypeHashValueIndex,
    ...
    for (int i = 0; i < loop_num; i++) {
      ...
      for (int id = 0; id < local_gpu_count; id++) {
        ...
        if (tile_count > 0) {
          ...
          memset_liner((*device_resources)[id]->get_stream(), value_index_buf,
                       (TypeHashKey)value_index_offset, (TypeHashKey)1, tile_count);
        }
   ...
   * @param context gpu device context, for switching device
   */
  void get_forward_results(int memcpy_size, const Tensors<float> &embedding_feature_tensors,
                           float *embedding_feature, Tensors<float> &temp_tensors,
                           const std::shared_ptr<GPUResourceGroup> &device_resources,
                           const CudaDeviceContext &context) {
    int local_gpu_count = device_resources->size();
    int total_gpu_count = device_resources->get_total_gpu_count();
   ...
   * @param context gpu device context, for switching device
   */
  void get_backward_results(int devId, int memcpy_size, const Tensors<float> &wgrad_tensors,
                            float *wgrad, const std::shared_ptr<GPUResourceGroup> &device_resources,
                            const CudaDeviceContext &context) {
  ...
  void get_update_params_results(
      size_t max_vocabulary_size_per_gpu, int embedding_vec_size, size_t vocabulary_size,
      const Tensors<float> &hash_table_value_tensors,
      const std::vector<std::unique_ptr<
          HashTable<TypeHashKey, TypeHashValueIndex, std::numeric_limits<TypeHashKey>::max()>>>
          &hash_tables,
