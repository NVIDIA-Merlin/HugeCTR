...
class SingleDataSimulator : public DataSimulator<T> {
  public:
    SingleDataSimulator(std::function<T()> func)
    : func_(func) {}
...
class VarianceScalingSimulator : public DataSimulator<T> {
  public:
    VarianceScalingSimulator(float scale, data_simu::Mode_t mode, data_simu::Distribution_t distribution, 
                            float in_dim, float out_dim, bool truncated = true)
