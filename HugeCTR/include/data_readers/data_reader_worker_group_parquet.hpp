...
class DataReaderWorkerGroupParquet : public DataReaderWorkerGroup {
  std::shared_ptr<Source> create_source(size_t worker_id, size_t num_worker,
                                        const std::string& file_name,bool strict_order_of_batches, bool repeat) override {
    return std::make_shared<ParquetFileSource>(worker_id, num_worker, file_name, strict_order_of_batches,repeat);
  }
 ...
 public:
  DataReaderWorkerGroupParquet(const std::vector<std::shared_ptr<ThreadBuffer>>& output_buffers,
    ...
    if (num_workers > local_gpu_count) {
      HCTR_OWN_THROW(Error_t::WrongInput, "parquet workers should be no greater than local_gpu_count");
    }
    ...
    for (size_t i = 0; i < num_workers; i++) {
      std::shared_ptr<IDataReaderWorker> data_reader(new ParquetDataReaderWorker<TypeKey>(
          ...
          &data_reader_loop_flag_, output_buffers[i], file_list, strict_order_of_batches,repeat, params, slot_offset,
