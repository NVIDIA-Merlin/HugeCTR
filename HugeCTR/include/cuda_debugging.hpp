...
#define HCTR_CUDA_KERNEL_SUMMARY(MODE, FUNC, BLOCKSIZE, DSHMEM)                       \
  do {                                                                                \
    ...
    HCTR_CUDA_CHECK(ASYNC, cudaOccupancyMaxActiveBlocksPerMultiprocessor(             \
                           &max_blocks,                                               \
                           FUNC,                                                      \
                           BLOCKSIZE,                                                 \
                           DSHMEM));                                                  \
    HCTR_LOG(MODE, WORLD, #FUNC " Summary:\n"                                         \
                          "\tbinaryVersion:             %d\n"                         \
                          "\tcacheModeCA:               %d\n"                         \
                          "\tconstSizeBytes:            %zu\n"                        \
                          "\tlocalSizeBytes:            %zu\n"                        \
                          ...
                          "\tblock size:                %d\n"                         \
                          "\tmax blocks per SM:         %d\n",                        \
                          func_attr.binaryVersion,                                    \
                          func_attr.cacheModeCA,                                      \
                          func_attr.constSizeBytes,                                   \
                          func_attr.localSizeBytes,                                   \
                          func_attr.maxDynamicSharedSizeBytes,                        \
                          func_attr.maxThreadsPerBlock,                               \
                          func_attr.numRegs,                                          \
                          func_attr.preferredShmemCarveout,                           \
                          func_attr.ptxVersion,                                       \
                          func_attr.sharedSizeBytes,                                  \
                          BLOCKSIZE,                                                  \
                          max_blocks);                                                \
  } while (0)
...
#define HCTR_CUDA_KERNEL_TIME(MODE, KERNEL_LAUNCH, ITER)                              \
  ...
  {                                                                                   \
    ...
    HCTR_LOG(MODE, WORLD, #KERNEL_LAUNCH ":\n"                                        \
