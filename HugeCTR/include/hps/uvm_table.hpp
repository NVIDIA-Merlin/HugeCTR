...
class UvmTable : public EmbeddingCacheBase,
                 public std::enable_shared_from_this<UvmTable<TypeHashKey>> {
 public:
  virtual ~UvmTable();
  UvmTable(const InferenceParams& inference_params, const parameter_server_config& ps_config,
           HierParameterServerBase* const parameter_server);
  UvmTable(UvmTable const&) = delete;
  UvmTable& operator=(UvmTable const&) = delete;
 ...
 private:
  using Cache = gpu_cache::UvmTable<TypeHashKey, size_t>;
  ...
  // embedding tables
  std::vector<std::unique_ptr<Cache>> uvm_tables_;
