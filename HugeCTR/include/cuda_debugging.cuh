 ...
 * 1.1. output: 
 * 
     [HUGECTR][KERNEL][device 0, grid 36, block (427,0,0), thread (0,0,0), sm 39, warp 1, lane 0] The used value: 64
     [HUGECTR][KERNEL][device 0, grid 36, block (469,0,0), thread (0,0,0), sm 56, warp 0, lane 0] The used value: 64
     [HUGECTR][KERNEL][device 0, grid 36, block (421,0,0), thread (0,0,0), sm 71, warp 0, lane 0] The used value: 64
     [HUGECTR][KERNEL][device 0, grid 36, block (389,0,0), thread (0,0,0), sm 43, warp 0, lane 0] The used value: 64
     [HUGECTR][KERNEL][device 0, grid 36, block (445,0,0), thread (0,0,0), sm 71, warp 1, lane 0] The used value: 64
     [HUGECTR][KERNEL][device 0, grid 36, block (417,0,0), thread (0,0,0), sm 9, warp 0, lane 0] The used value: 64
 *
 ...
 * 2. It is not a non-destructive test to insert printfs because its overuse can affect the kernel register usage,
 * warp scheduling, the amounts of computation, etc. It is up to users to limit the amount of printed messages.
 * It is also recommended that, once you finish debugging your kernel, remove or comment out all the printfs.
 * 3. Currently, the maximum length of a printed message is 256 including the prefix (See KERNEL_PRINTF_MAX_LENGTH below).
...
// The maximum length of printed message. It can affect the SM resource usage and performance to set it to a larger value.
...
__device__ void kernel_printf(const char* format, Type1 arg1, Type2 arg2, Type3 arg3, Type4 arg4, Type5 arg5) {
    char extended_format[KERNEL_PRINTF_MAX_LENGTH] = {};
    kernel_printf_extend_format(extended_format, format);
...
__device__ void kernel_printf(const char* format, Type1 arg1, Type2 arg2, Type3 arg3, Type4 arg4, Type5 arg5, Type6 arg6) {
    char extended_format[KERNEL_PRINTF_MAX_LENGTH] = {};
    kernel_printf_extend_format(extended_format, format);
...
__device__ void kernel_printf(const char* format, Type1 arg1, Type2 arg2, Type3 arg3, Type4 arg4, Type5 arg5, Type6 arg6, Type7 arg7) {
    char extended_format[KERNEL_PRINTF_MAX_LENGTH] = {};
    kernel_printf_extend_format(extended_format, format);
 ...
 */
#define HCTR_KERNEL_PRINT(...) do { kernel_printf(__VA_ARGS__); } while (0)
#define HCTR_KERNEL_PRINT_IF(cond, ...) do { if (cond) { HCTR_KERNEL_PRINT(__VA_ARGS__); } } while (0)
#define HCTR_KERNEL_PRINT_PER_BLOCK(...) do { HCTR_KERNEL_PRINT_IF((threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0), __VA_ARGS__); } while (0)
#define HCTR_KERNEL_PRINT_PER_GRID(...) do { if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) { HCTR_KERNEL_PRINT_PER_BLOCK(__VA_ARGS__); } } while (0)
