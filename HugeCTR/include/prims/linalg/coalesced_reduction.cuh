...
// values of dots.
template <typename InType, typename OutType, typename IdxType, int TPB, typename MainLambda,
          typename ReduceLambda, typename FinalLambda>
__global__ void coalescedReductionKernel(OutType *dots, InType **data, int D, int N, OutType init,
                                         MainLambda main_op, ReduceLambda reduce_op,
                                         FinalLambda final_op, bool rowMajor,
                                         bool inplace = false) {
  ...
  if (rowMajor) {
    for (IdxType i = threadIdx.x; i < D; i += TPB) {
      thread_data = reduce_op(thread_data, main_op(data[blockIdx.x][i], i));
    }
  } else {
    for (IdxType i = threadIdx.x; i < N; i += TPB) {
      thread_data = reduce_op(thread_data, main_op(data[i][blockIdx.x], i));
    }
  }
...
void coalescedReduction(OutType *dots, InType **data, int D, int N, OutType init,
  ...
  if (TS <= 32) {
    coalescedReductionKernel<InType, OutType, IdxType, 32><<<BS, 32, 0, stream>>>(
        dots, data, D, N, init, main_op, reduce_op, final_op, rowMajor, inplace);
  } else if (TS <= 64) {
    coalescedReductionKernel<InType, OutType, IdxType, 64><<<BS, 64, 0, stream>>>(
        dots, data, D, N, init, main_op, reduce_op, final_op, rowMajor, inplace);
  } else if (TS <= 128) {
    coalescedReductionKernel<InType, OutType, IdxType, 128><<<BS, 128, 0, stream>>>(
        dots, data, D, N, init, main_op, reduce_op, final_op, rowMajor, inplace);
  } else {
    coalescedReductionKernel<InType, OutType, IdxType, 256><<<BS, 256, 0, stream>>>(
        dots, data, D, N, init, main_op, reduce_op, final_op, rowMajor, inplace);
  }
