 ...
 */
 ...
 template <typename InType1, typename InType2, int VecLen, typename Lambda, typename IdxType,
           typename OutType>
 __global__ void binaryOpKernel(OutType *out, const InType1 *in1,
                                const InType2 *in2, IdxType len, Lambda op) {
 ...
 void binaryOpImpl(OutType *out, const InType1 *in1, const InType2 *in2,
 ...
 void binaryOp(OutType *out, const InType1 *in1, const InType2 *in2, IdxType len,
   ...
   if (16 / maxSize && bytes % 16 == 0) {
     binaryOpImpl<InType1, InType2, 16 / maxSize, Lambda, IdxType, OutType, TPB>(
       out, in1, in2, len, op, stream);
   } else if (8 / maxSize && bytes % 8 == 0) {
     binaryOpImpl<InType1, InType2, 8 / maxSize, Lambda, IdxType, OutType, TPB>(
       out, in1, in2, len, op, stream);
   } else if (4 / maxSize && bytes % 4 == 0) {
     binaryOpImpl<InType1, InType2, 4 / maxSize, Lambda, IdxType, OutType, TPB>(
       out, in1, in2, len, op, stream);
   } else if (2 / maxSize && bytes % 2 == 0) {
     binaryOpImpl<InType1, InType2, 2 / maxSize, Lambda, IdxType, OutType, TPB>(
       out, in1, in2, len, op, stream);
   } else if (1 / maxSize) {
     binaryOpImpl<InType1, InType2, 1 / maxSize, Lambda, IdxType, OutType, TPB>(
       out, in1, in2, len, op, stream);
   } else {
     binaryOpImpl<InType1, InType2, 1, Lambda, IdxType, OutType, TPB>(out, in1, in2, len,
                                                            op, stream);
   }
 }
