...
template <typename T>
void dot_product_cpu(T **input, T *output, int size, int num) {
...
template <typename T>
void dot_product_dgrad_cpu(const T *top_grad, T **dgrad, const T *fprop_output, int size, int num) {
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < num; j++){
      if (0 == fprop_output[i]){
        dgrad[j][i] = 0;
...
void dot_product_test(int batch_size, int slot_num, int embedding_vec_size, int num) {
  int dev_id = 0;
