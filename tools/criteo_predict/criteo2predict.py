...
def convert(src_csv, src_config, dst, batch_size):
    ...
    with open(dst, 'w') as dst_txt:
        label_df.to_csv(dst_txt, mode='a+', sep = ' ', index=False,  header=False)
        dense_df.to_csv(dst_txt, mode='a+', sep = ' ', index=False,  header=False)
        embedding_columns_df.to_csv(dst_txt, mode='a+', sep = ' ', index=False,  header=False)
        row_ptrs_df.to_csv(dst_txt, mode='a+', sep = ' ', index=False,  header=False)
...
if __name__ == '__main__':
    ...
    convert(src_csv_path, src_config_path, dst_path, batch_size)