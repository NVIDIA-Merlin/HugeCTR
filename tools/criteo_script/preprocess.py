...
def _merge_and_transform_series(src_series_list, col, dense_cols, normalize_dense):
    ...
    if col != "label":
        ...
        result_series = result_series.apply(lambda x: x if x in unique_value_counts else NAN_VALUE)
...
def _merge_columns_and_feature_cross(series_list, min_max, feature_pairs, feature_cross):
    name_to_series = dict()
    ...
    if feature_cross != 0:
        for idx, pair in enumerate(feature_pairs):
            ...
            oe = skp.OrdinalEncoder(dtype=np.int64)
            crossed_column_series = pd.DataFrame(
                oe.fit_transform(pd.DataFrame(crossed_column_series))
            )
            ...
            logging.info(
                "column {} [{}, {}]".format(
                    crossed_column, str(df[crossed_column].min()), str(crossed_column_max_val)
                )
            )
...
def preprocess(src_txt_name, dst_txt_name, normalize_dense, feature_cross):
    ...
    with cf.ThreadPoolExecutor(max_workers=MAX_NUM_WORKERS) as executor:
        ...
        reader = pd.read_csv(src_txt_name, sep="\t", names=cols, chunksize=131072)

        logging.info("_fill_missing_features_and_split")
        for col in cols:
            series_list_dict[col] = list()
        _process_chunks(executor, reader, _fill_missing_features_and_split, series_list_dict)
    ...
    with cf.ProcessPoolExecutor(max_workers=MAX_NUM_WORKERS) as executor:
        ...
        for col, src_series_list in series_list_dict.items():
            future = executor.submit(
                _merge_and_transform_series, src_series_list, col, dense_cols, normalize_dense
            )
            futures.append(future)
        ...
        df = _merge_columns_and_feature_cross(
            dst_series_list, min_max, feature_pairs, feature_cross
        )
