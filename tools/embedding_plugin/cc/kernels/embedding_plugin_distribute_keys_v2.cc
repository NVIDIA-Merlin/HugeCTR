...
/* 
This op distribute keys to CSR format at GPU.
TODO: But need more modification. Cause in TF's datareader preprocess, multi-thread will be used
to do preprocessing, therefore each CPU-thread should have its own hardware resources and 
memory objectes.
The outputs will be a list of row_offsets, a list of value_tensors, nnz_array.
*/
template <typename Device>
class EmbeddingDistributeKeysV2Op : public OpKernel {
public:
    explicit EmbeddingDistributeKeysV2Op(OpKernelConstruction* ctx) : OpKernel(ctx) {
...
REGISTER_KERNEL_BUILDER(Name("HugectrEmbeddingDistributeKeysV2").Device(DEVICE_CPU), 
                        EmbeddingDistributeKeysV2Op<CPUDevice>);
