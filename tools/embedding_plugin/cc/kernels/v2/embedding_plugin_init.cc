...
public:
    explicit InitOp(OpKernelConstruction* ctx) : OpKernel(ctx){
        ...
        OP_REQUIRES_OK(ctx, ctx->GetAttr("key_type", &HugeCTR::Version2::key_type));
        OP_REQUIRES_OK(ctx, ctx->GetAttr("value_type", &HugeCTR::Version2::value_type));
        OP_REQUIRES(ctx, Utils::check_in_set(HugeCTR::Version2::KEY_TYPE_SET, HugeCTR::Version2::key_type),
                errors::InvalidArgument("key_type must be {uint32, int64}."));
        OP_REQUIRES(ctx, Utils::check_in_set(HugeCTR::Version2::VALUE_TYPE_SET, HugeCTR::Version2::value_type),
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        OP_REQUIRES_OK(ctx, ctx->input("visible_gpus", &vvgpu));
        /*check dims, should be {1, 2}*/
        auto vvgpu_flat = vvgpu->flat<int32>();
        OP_REQUIRES(ctx, batch_size_ % vvgpu_flat.size() == 0, errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                                "batch_size should be multiple of visible_gpus."));
        OP_REQUIRES(ctx, batch_size_eval_ % vvgpu_flat.size() == 0, errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                                "batch_size_eval should be multiple of visible_gpus."));
            ...
            for (auto gpu : vec) {
                if (gpu > before) {
                    ...
                    ctx->SetStatus(errors::InvalidArgument(__FILE__, ":", __LINE__, " ",
                                  "visible_gpus for embedding plugin is not valid. They must be in ascending order",
                                  " and without repetition."));
                    return;
                }
            }
        }
        LOG(INFO) << "visible_gpus for embedding plugin: " << show_gpus;
        ...
        if (HugeCTR::Version2::wrapper) {
            ...
            if ("uint32" == HugeCTR::Version2::key_type) {
                if ("float" == HugeCTR::Version2::value_type) {
                    HugeCTR::Version2::wrapper.reset(new HugeCTR::Version2::EmbeddingWrapper<unsigned int, float>(vecvecgpu,
                                                   static_cast<unsigned long long>(seed_),
                                                   batch_size_, batch_size_eval_));
                } else if ("half" == HugeCTR::Version2::value_type) {
                    HugeCTR::Version2::wrapper.reset(new HugeCTR::Version2::EmbeddingWrapper<unsigned int, __half>(vecvecgpu,
                                                    static_cast<unsigned long long>(seed_),
                                                    batch_size_, batch_size_eval_));
                }
            } else if ("int64" == HugeCTR::Version2::key_type) {
                if ("float" == HugeCTR::Version2::value_type) {
                    HugeCTR::Version2::wrapper.reset(new HugeCTR::Version2::EmbeddingWrapper<long long, float>(vecvecgpu,
                                                    static_cast<unsigned long long>(seed_),
                                                    batch_size_, batch_size_eval_));
                } else if ("half" == HugeCTR::Version2::value_type) {
                    HugeCTR::Version2::wrapper.reset(new HugeCTR::Version2::EmbeddingWrapper<long long, __half>(vecvecgpu,
                                                    static_cast<unsigned long long>(seed_),
                                                    batch_size_, batch_size_eval_));
                }
            }
...
REGISTER_KERNEL_BUILDER(Name("V2HugectrInit").Device(DEVICE_CPU), InitOp<CPUDevice>);
REGISTER_KERNEL_BUILDER(Name("V2HugectrInit").Device(DEVICE_GPU), InitOp<GPUDevice>);
