...
+ **Host memory cache for HugeCTR embedding training cache**: We have introduced the host memory cache PS (Cached-PS) for the incremental training, which is a component of the [Embedding Training Cache]( ./docs/hugectr_embedding_training_cache.md) (ETC) and responsible for handling the case when the embedding table is too large to even fit into the **host memory**. We have provided the SSD-based PS to handle this scenario in former releases, but the SSD-based PS will be deprecated from the v3.2.1 release due to its unsatisfactory performance. Please check the [Cached Host Memory Parameter Server](./docs/hugectr_embedding_training_cache.md#32-cached-host-memory-parameter-server) for a detailed introduction.</br>
Compared with the former SSD-based PS, the loading and dumping bandwidth of the Cached-PS can be substantially improved if it is properly configured, which contributes to the incremental training of models with huge embedding tables when using the ETC feature in HugeCTR.</br>
To ease the utilization of the ETC feature, we have also simplified the python interface of ETC. Specifically, we drop the `use_host_memory_ps` entry by providing the `ps_types` entry for choosing the `Staged` PS or the `Cached` PS; A unified entry `sparse_models` is introduced, and you don’t need to use different entries to tell whether a pre-trained embedding table exists or not. For a detailed explanation of the python interface, please check the [HugeCTR Python Interface](./docs/python_interface.md#createetc-method).
